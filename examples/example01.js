...
function init() {
    ...
    chain = client.newChain(config.chainID);
...
function end2end() {
    ...
    hfc.newDefaultKeyValueStore({
        path: __dirname + config.keyValueStore
    }).then(function(store) {
        client.setStateStore(store);
        var users = config.users;
        var promise = getSubmitter(users[0].username, users[0].secret, client);
            ...
            promise = promise.then(
                function(admin) {
                    ...
                    var request = {
                        ...
                        txId: tx_id,
                        nonce: nonce,
                        //TODO: Should we move this to some default config ?
                ...
                promise = promise.then(
                    function(response) {
                        if (response.status === 'SUCCESS') {
                            ...
                            return sleep(config.sleepDuration);
                        } else {
            ...
            return new Promise((resolve, reject) => {
                if (user && user.isEnrolled()) {
                    ...
                    if (!loadFromConfig) {
                        ...
                        var cop = new copService(config.caserver.ca_url);
                        ...
                        var privKeyPEM = path.join(__dirname, config.privKeyPEM);
                        var pemData;
                        return readFile(privKeyPEM)
                            .then((data) => {
                                ...
                                var certPEM = path.join(__dirname, config.certPEM);
...
function getArgs(ccArgs) {
    var args = [];
    for (var i = 0; i < ccArgs.length; i++) {
        args.push(ccArgs[i]);
    }
    return args;
}
