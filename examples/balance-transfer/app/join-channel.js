...
var joinChannel = function(channelName, peers, username, org) {
	...
	return helper.getRegisteredUsers(username, org).then((member) => {
		...
		if (results[0] && results[0][0] && results[0][0].response && results[0][0]
			.response.status == 200) {
			logger.info(util.format(
				'Successfully joined peers in organization %s to join the channel',
				org));
			closeConnections(true);
			return util.format(
				'Successfully joined peers in organization %s to join the channel', org
			);
		} else {
		...
		logger.error('Failed to join channel due to error: ' + err.stack ? err.stack :
			err);
		closeConnections();
		return 'Failed to join channel due to error: ' + err.stack ? err.stack :
			err;
	});
};
