...
class Peer extends Remote {
	...
	constructor(name = checkParameter('name'), client = checkParameter('client'), mspid) {
		logger.debug(`constructor[${name}] - start `);
		super(name, client);
		this.mspid = mspid;
		this.type = TYPE;
	...
	async connect(endpoint = checkParameter('endpoint'), options = {}) {
		...
		if (this.endorserService) {
			logger.debug(`${method} - endorser service exist, will close this peer ${this.name}`);
			this.close();
		} else {
	 ...
	 * @param {boolean} [forceReconnect] when truthy while `this.connected` is also truthy, reconnect action will be taken.
	 * @return {boolean} connection status
	 */
	async checkConnection(forceReconnect) {
