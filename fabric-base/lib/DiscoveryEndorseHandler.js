...
const logger = utils.getLogger('DiscoveryEndorseHandler');
...
/**
 * This is an implementation for a endorsement handler. The only requirement is to
 * have a endorse() method.
 * It will submit transactions to be endorsed to a target list generated from the
 * results of service discovery.
 *
 * @class
 */
class DiscoveryEndorseHandler {
	...
	async endorse(request = checkParameter('checkParameter'), signed_proposal = checkParameter('signed_proposal')) {
		...
		let results = await this.discovery.getDiscoveryResults(true);

		if (results && results.endorsement_plan) {

			return this._endorse(results.endorsement_plan, request, signed_proposal, timeout);
		} else {
		...
		return new Promise(async (resolve) => {
			for (const peer_info of group.peers) {
				...
				if (previous_endorsement && previous_endorsement.endorsement) {
					...
					const peer = self.discovery.channel.getPeer(peer_info.name);
					if (peer) {
...
module.exports = DiscoveryEndorseHandler;
