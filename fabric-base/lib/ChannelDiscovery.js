...
class ChannelDiscovery extends Remote {
	...
	newEndorsementHandler() {
		const method = 'newEndorsementHandler';
		logger.debug('%s - start', method);
		const handler = new DiscoveryEndorsementHandler(this);
	...
	async _buildPeer(discovery_peer, msp) {
		...
		const found = this.channel.getPeer(address); //addresss is used as name
		if (found) {
		...
		if (msp_id && msp) {
			...
			const peer = this.client.newPeer(discovery_peer.name, msp_id);
