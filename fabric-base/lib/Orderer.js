...
class Orderer extends Remote {
	...
	async checkConnection() {
		...
		if (this.connected) {
			try {
				...
				logger.error('Orderer %s Connection failed :: %s', this.endpoint.url, error);
			}
		}
	...
	disconnect() {
		...
		if (this.broadcastService) {
			logger.debug('%s - closing orderer broadcast connection %s', method, this.endpoint.addr);
	...
	sendBroadcast(envelope = checkParameter('envelope'), timeout) {
		...
		return new Promise((resolve, reject) => {
			...
			const broadcast_timeout = setTimeout(() => {
				logger.error('%s - %s timed out after:%s', self.name, method, rto);
				broadcast.end();
			...
			broadcast.on('data', (response) => {
				...
				if (response && response.info) {
					logger.debug('%s - response info :: %s', method, response.info);
				}
				if (response && response.status) {
					logger.debug('%s - response status %s', method, response.status);
					return resolve(response);
				} else {
					logger.error('%s ERROR - %s reject with invalid response from the orderer', self.this.name, method);
					return reject(new Error('SYSTEM_ERROR'));
				}
			});

			broadcast.on('end', () => {
				logger.debug('%s - on end:', method);
			...
			broadcast.on('error', (err) => {
				...
				if (err && err.code) {
					if (err.code === 14) {
						logger.error('%s - %s on error: %j', method, self.name, err.stack ? err.stack : err);
			...
			logger.debug('%s - sent message', method);
		});
	}
