...
class Discovery extends Remote {
	 ...
	 * @property {string} channelId - The Channel to discovery config and
	 *  endorsement plans.
	 ...
	 */
	buildRequest(txContext = checkParameter('txContext'), request = checkParameter('request')) {
		const method = 'buildDiscoveryRequest';
		logger.debug('%s - start', method);
	
		// always get the config, we need the msps, do not need local
		const {config = true,  local = false, channelId = checkParameter('channelId'), interests} = request;
	...
	async _buildPeer(discovery_peer, msp) {
		...
		if (found) {
			logger.debug('%s - found peer %s', method, address);
			return;
		}
