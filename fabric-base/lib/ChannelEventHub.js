...
class ChannelEventHub extends ServiceEndpoint {
	...
	_disconnect(err) {
		...
		logger.debug(`${method} - start - called due to:: ${err.message}, peer:${this.endpoint.url}`);

		if (this._disconnect_running) {
			logger.debug(`${method} - disconnect is running - exiting`);
			return;
		}
	...
	buildRequest(idContext = checkParameter('idContext'), options = {}) {
		...
		const transactionId = idContext.newTransactionId();
		const nonce = idContext.newNonce();
		...
		const seekHeader = buildHeader(nonce, channelHeader);
	...
	listen(timeout) {
		...
		logger.debug(`${method} - start stream:${stream_id}`);
		...
		const connection_setup_timeout = setTimeout(() => {
			...
			this._disconnect(new Error('TIMEOUT - Unable to receive blocks from the fabric peer event service'));
		}, rto);
		...
		this.stream.on('data', (deliverResponse) => {
			logger.debug(`on.data - block stream:${stream_id} _current_stream:${this._current_stream} peer:${this.endpoint.url}`);
			if (stream_id !== this._current_stream) {
			...
			if (this._stream_starting) {
				logger.debug(`on.data - stream ${stream_id} now started, block number ${this.last_block_number}`);
				this._stream_starting = false;
			...
			if (deliverResponse.Type === 'block' || deliverResponse.Type === 'filtered_block' || deliverResponse.Type === 'private_data') {
				try {
					...
					if (this.end_block) {
						if (this.end_block.lessThanOrEqual(this.last_block_number)) {
							...
							this._disconnect(new Error(`Shutdown due to end block number has been seen: ${this.last_block_number}`));
						}
					}
				} catch (error) {
					logger.error(`${method} ChannelEventHub - ::${error.stack ? error.stack : error}`);
					logger.error(`${method} ChannelEventHub has detected an error ${error.toString()}`);
				...
				if (deliverResponse.status === 'SUCCESS') {
					if (this.end_block_seen) {
						...
						logger.debug(`on.data - status received after last block seen: ${deliverResponse.status} block_num:${this.last_block_number}`);
					}
					if (this.ending_block === NEWEST) {
						// this is normal after the last block comes in when we set to newest as an ending block
						logger.debug(`on.data - status received when newest block seen: ${deliverResponse.status} block_num:${this.last_block_number}`);
						this._disconnect(new Error(`Newest block received:${this.last_block_number} status:${deliverResponse.status}`));
					}
				} else {
					// tell all registered users that something is wrong and shutting down
					logger.error(`on.data - unexpected deliverResponse status received - ${deliverResponse.status}`);
					this._disconnect(new Error(`Event stream has received an unexpected status message. status:${deliverResponse.status}`));
				}
			} else {
				logger.error(`on.data - unknown deliverResponse type ${deliverResponse.Type}`);
				this._disconnect(new Error(`Event stream has received an unknown response type ${deliverResponse.Type}`));
			}
		});

		this.stream.on('status', (response) => {
			logger.debug(`on status - status received: ${JSON.stringify(response)}  peer:${this.endpoint.url}`);
			if (this._stream_starting) {
				logger.debug(`on.status - stream ${stream_id} not started`);
			}
		});

		this.stream.on('end', () => {
			logger.debug(`on.end - event stream:${stream_id} _current_stream:${this._current_stream} peer:${this.endpoint.url}`);
			if (stream_id !== this._current_stream) {
			...
			// tell all registered users that something is wrong and shutting down
			logger.debug(`on.end - grpc stream is ready :${this.isStreamReady()}`);
			this._disconnect(new Error('fabric peer service has disconnected due to an "end" event'));
		});

		this.stream.on('error', (err) => {
			logger.debug(`on.error - block stream:${stream_id} _current_stream:${this._current_stream}  peer:${this.endpoint.url}`);
			if (stream_id !== this._current_stream) {
				logger.debug('on.error - incoming message was from a cancelled stream');
				logger.debug(`on.error - ${new Date()} ${err}`);
				return;
			}
			if (this._stream_starting) {
				logger.debug(`on.status - stream ${stream_id} not started`);
			...
			// tell all registered users that something is wrong and shutting down
			logger.debug(`on.error - grpc stream ready state :${this.isStreamReady()}`);
			if (err instanceof Error) {
				this._disconnect(err);
			} else {
				this._disconnect(new Error(err));
			}
		});

		this.stream.write(envelope);

		logger.debug(`${method} - end - stream_id:${stream_id}`);
	}
	...
	_closeAllCallbacks(err) {
		...
		logger.debug(`${method} - event registrations ${this._eventRegistrations.size}`);
		for (const event_reg of this._eventRegistrations.values()) {
			logger.debug(`${method} - closing event registration:${event_reg.id} type:${event_reg.type}`);
	...
	unregisterEventListener(registrationId = 'all') {
		const method = `checkConnection[${this.name}]`;
		logger.debug(`${method} - start - registrationId:${registrationId}`);
	...
	_processEndBlock(block_num) {
		...
		for (const any_reg of this._eventRegistrations.values()) {
			if (any_reg.end_block >= block_num) {
				...
				logger.debug(`${method} - automatically unregister listener ${any_reg.id} received defined end block ${block_num}`);
			}
		}
	}
		...
		for (const block_reg of this._eventRegistrations.values()) {
			if (block_reg.type === BLOCK) {
				...
				if (block_reg.unregister && event.endBlockReceived) {
					...
					logger.debug(`${method} - automatically unregister block listener for ${block_reg.id}`);
				}
			}
		}
	}
