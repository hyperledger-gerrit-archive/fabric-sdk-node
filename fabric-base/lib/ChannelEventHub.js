...
class ChannelEventHub extends ServiceEndpoint {
	...
	listen(timeout) {
		...
		this.stream.on('status', (response) => {
			logger.debug('on status - status received: %j  peer:%s', response, this.endpoint.url);
			if (this._stream_starting) {
		...
		this.stream.on('end', () => {
			logger.debug('on.end - event stream:%s _current_stream:%s peer:%s', stream_id, this._current_stream, this.endpoint.url);
			if (stream_id !== this._current_stream) {
		...
		this.stream.on('error', (err) => {
			logger.debug('on.error - block stream:%s _current_stream:%s  peer:%s', stream_id, this._current_stream, this.endpoint.url);
			if (stream_id !== this._current_stream) {
