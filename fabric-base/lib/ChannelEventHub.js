...
class ChannelEventHub  extends Remote {
	...
	buildStartRequest(txContext = cp('txContext'), options = {}) {
		const method = `buildStartRequest[${this.name}]`;
		logger.debug('%s - start', method);
		...
		const seekHeader = buildHeader(txContext, channelHeader);
	...
	startListening(timeout) {
		const method = `startListening[${this.name}]`;
		logger.debug('%s - start', method);
		if (!this.connect) {
		...
		const envelope = this.getSignedStartRequestEnvelope();
		this.end_block_seen = false;
		...
		if (this.filtered) {
			this._stream = this.event_service.deliverFiltered();
		} else {
			this._stream = this.event_service.deliver();
		}
	...
	async checkConnection() {
		...
		logger.debug('%s - start', method);
		if (this.event_service && this._stream) {
			try {
				await this.waitForReady(this.event_service);
				...
				logger.error('Event Service %s Connection failed :: %s', this.endpoint.url, error);
			}
		...
		for (const event_reg of this._eventRegistrations.values()) {
			if (event_reg.type === BLOCK) {
				...
				if (block_reg.disconnect) {
					...
					this._disconnect(new Error('Shutdown due to disconnect on block registration'));
				}
			}
		}
	}
	...
	_processChaincodeEvents(block) {
		...
		for (const [chaincode_reg, events] of all_events.entries()) {
			...
			if (chaincode_reg.disconnect) {
				...
				this._disconnect(new Error('Shutdown due to disconnect on chaincode event registration'));
			}
		}
	}
