...
const Channel = class {
	...
	addPeer(peer, roles, replace) {
		...
		logger.debug('/n adding a new peer  --name: %s --URL: %s', name, peer.endpoint.url);

		const channel_peer = new ChannelPeer(this, peer, roles);
	...
	removeOrderer(orderer) {
		orderer.close();
		this.orderers.delete(orderer.getName());
	}
	...
	/**
	 * Will return an array of {@link ChannelPeer} instances that have been
	 ...
	 */
	getChannelDiscovery() {
		const method = 'getChannelDiscovery';
		logger.debug('%s start', method);

		if (!this.channelDiscovery) {
			logger.debug('%s create new channel discovery name:%s', method, this.name);
			this.channelDiscovery = new ChannelDiscovery(this.name, this.client, this);
		}
		
		logger.debug('%s return new channel discovery name:%s', method, this.name);
...
const ChannelPeer = class {
	 ...
	 */
	constructor(channel, peer, roles) {
		this.mspid = peer.mspid;
		if (channel && channel.type === 'Channel') {
			if (peer && peer.type === 'Peer') {
