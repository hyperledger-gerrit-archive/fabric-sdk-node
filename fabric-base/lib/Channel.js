...
const Channel = class {
	...
	getMSP(name = checkParameter('name')) {
		const method = `getMSP[${this.name}]`;
		logger.debug('%s - start name:%s', method, name);

		const msp = this.msps.get(name);
	...
	removeMSP(name = checkParameter('name')) {
		const method = `removeMSP[${this.name}]`;
		logger.debug('%s - start', method);
	...
	addPeer(peer, replace) {
		...
		if (check) {
			if (replace) {
				logger.debug('%s - removing existing Peer  --name: %s --URL: %s', method, check.name, check.endpoint.url);
		...
		logger.debug('%s - adding a new Peer  --name: %s', method, peer.name);
	...
	newChannelEventHub(name = checkParameter('name'), mspid) {
		...
		logger.debug('%s - start', method);
		const channel_event_hub = new ChannelEventHub(name, this, mspid);
	 ...
	 * <br>*** note *** This ChannelEventHub must be connected to an endpoint
	 *
	 * @param {string} name - The name for this ChannelEventHub
	 * @param {string} mspid - The organization for this Channel Event Hub
	 * @returns {ChannelEventHub} The ChannelEventHub instance
	...
	getChannelEventHub(name = checkParameter('name'), mspid) {
		...
		if (!channel_event_hub) {
			...
			if (mspid) {
				if (channel_event_hub.mspid) {
					if (mspid === channel_event_hub.mspid) {
						logger.debug('%s - mspids are the same %s', method, mspid);
					} else {
						throw Error(`MSPids do not match requested:${mspid} existing:${channel_event_hub.mspid}`);
					}
				} else {
					channel_event_hub.mspid = mspid;
					logger.debug('%s - assigning mspid %s to the channel event hub %s', method, mspid, channel_event_hub.name);
				}
			} else {
				logger.debug('%s - no mspid requested for this channel event hub %s', method, channel_event_hub.name);
			}
		}
