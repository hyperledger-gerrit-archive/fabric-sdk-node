...
const Channel = class {
	...
	async newChannelEventHubforPeer(name = checkParameter('name'), options) {
		...
		if (peer) {
			...
			try {
				logger.debug(`${method} - will try to connect to peer event service for ${name}`);
				await hub.connect(peer.endpoint, options);
				return hub;
			} catch (error) {
				logger.error(`${method} - Unable to connect to the Peer event service ${error}`);
				throw error;
			}
		} else {
	...
	getChannelEventHub(name = checkParameter('name'), mspid) {
		...
		if (!channel_event_hub) {
			logger.debug(`${method} create channel_event_hub name:${name}`);
			channel_event_hub = new ChannelEventHub(name, this, mspid);
			this.channelEventHubs.set(name, channel_event_hub);
		}

		logger.debug(`${method} return channel_event_hub name:${name}`);
	...
	async newChannelEventHubsForOrg(mspid = checkParameter('mspid')) {
		...
		logger.debug(`${method} - starting - mspid: ${mspid}`);
	...
	newChannelDiscovery(name = checkParameter('name')) {
		...
		logger.debug(`${method} - start - create new channel discovery name:${name} for channel:${this.name}`);
