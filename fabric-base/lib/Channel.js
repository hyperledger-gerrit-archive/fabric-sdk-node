...
const ENDORSING_PEER_ROLE = 'endorsingPeer';
const CHAINCODE_QUERY_ROLE = 'chaincodeQuery';
const LEDGER_QUERY_ROLE = 'ledgerQuery';
const EVENT_SOURCE_ROLE = 'eventSource';
const DISCOVERY_ROLE = 'discover';
const ALL_ROLES = 'all';
...
const Channel = class {
	...
	constructor(name = checkParameter('name'), client = checkParameter('client')) {
		...
		this.name = name;
		this.client = client;
		this._peers = new Map(); //channel peers
	...
	newProposal(chaincode = checkParameter('chaincode')) {
		const method = 'getProposal';
		logger.debug('%s - start', method);

		return new Proposal(chaincode, this);
	}
		...
		if (targets) {
			...
			for (const peer of _targets) {
				if (typeof peer === 'string') {
					const channel_peer = this._channel_peers.get(peer);
					if (channel_peer) {
