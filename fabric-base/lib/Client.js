...
const Client = class extends BaseClient {
	...
	newEndpoint(opts = {}) {
		...
		if (typeof ssl_target_name_override === 'string') {
			...
			logger.debug(`${method} - ssl_target_name_override: ${ssl_target_name_override}`);
		}
		const endpoint = new Endpoint(options);
		logger.debug(`new endpoint url: ${options.url}`);
	...
	newPeer(name = checkParameter('name'), mspid) {
		...
		logger.debug(`${method} start name:${name}`);

		const peer = new Peer(name, this, mspid);

		logger.debug(`${method} return new peer name:${name}`);
		return peer;
	}
	...
	getPeer(name = checkParameter('name'), mspid) {
		...
		if (!peer) {
			logger.debug(`${method} create peer name:${name}`);
			peer = new Peer(name, this, mspid);
			this.peers.set(name, peer);
		} else {
			logger.debug(`${method} existing peer name:${name}`);
		}

		logger.debug(`${method} return peer name:${name}`);
		return peer;
	}
	/**
	...
	newOrderer(name = checkParameter('name'), mspid) {
		...
		logger.debug(`${method} return new orderer name:${name}`);
		return orderer;
	}
	...
	getOrderer(name = checkParameter('name'), mspid) {
		...
		if (!orderer) {
			logger.debug(`${method} create orderer name:${name}`);
			orderer = new Orderer(name, this, mspid);
			this.orderers.set(name, orderer);
		} else {
			logger.debug(`${method} existing orderer name:${name}`);
		}

		logger.debug(`${method} return orderer name:${name}`);
		return orderer;
	}
	...
	newChannel(name = checkParameter('name')) {
		...
		logger.debug(`${method} return new channel name:${name}`);
		return channel;
	}
	...
	getChannel(name = checkParameter('name')) {
		...
		if (!channel) {
			logger.debug(`${method} create channel name:${name}`);
			channel = new Channel(name, this);
			this.channels.set(name, channel);
		} else {
			logger.debug(`${method} existing channel name:${name}`);
		}

		logger.debug(`${method} return channel name:${name}`);
		return channel;
	}
	...
	setTlsClientCertAndKey(clientCert, clientKey) {
		...
		if (clientCert && clientKey) {
			...
			logger.debug(`${method} - generating self-signed TLS client certificate`);
