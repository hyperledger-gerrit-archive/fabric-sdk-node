...
const Client = class extends BaseClient {
	 ...
	 * @property {string} clientKey - The client private key file, in PEM format,
	 *  to use with the grpcs protocol and mutual TLS.
	 * @property {string} clientCert - The public certificate file, in PEM format,
	 *  to use with the grpcs protocol and mutual TLS.
	...
	getConnectionOptions(options) {
	  ...
	  * when connecting to peers, orderers, and channel event hubs. The returned
	  * instance must have an "url" for use when connecting to a fabric service.
	  * @param {ConnectOptions} opts 
	  */
	newEndpoint(opts = {}) {
		const method = 'newEndpoint';
		logger.debug('%s - start', method);
		...
		logger.debug('new endpoint url: %s, name: %s, options loaded are:: %j', this.url, this.name);
		
		return endpoint;
	}

	/**
	 * Builds a {@link TransactionContext} instance with the given user.
	 * Will be used when building proposals, commits, and querys.
	 * @param {User} user instance 
	 */
	newTransactionContext(user = checkParameter('user')) {
		return new TransactionContext(user, this);
	}
	...
	newDiscovery(name = checkParameter('name')) {
		const method = 'newDiscovery';
		logger.debug('%s start name:%s', method, name);

		const discovery = new Discovery(name, this);
		
		logger.debug('%s return new discovery name:%s', method, name);
		return discovery;
	}
	/**
