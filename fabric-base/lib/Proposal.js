...
class Proposal {
	 ...
	 */
	buildProposal(idContext = checkParameter('idContext'), request) {
		const method = `buildProposal[${this.chaincodeName}]`;
		logger.debug(`${method} - start`);
		this._endorsement = {type: 'endorsement'};

		return this._buildProposal(idContext, request, this._endorsement);
	}

	/*
	 * internal method to build proposal for both an endorsement and query
	 */
	_buildProposal(idContext, request, proposal) {
		const method = `_buildProposal[${this.chaincodeName}]`;
		logger.debug(`${method} - start - ${proposal.type}`);
		...
		if (fcn) {
			...
			logger.debug(`${method} - adding function arg:${proposal.fcn}`);
		} else {
			logger.debug(`${method} - not adding a function arg`);
		}

		for (const arg of args) {
			logger.debug(`${method} - adding arg ${arg}`);
			if (typeof arg === 'string') {
				proposal.args.push(Buffer.from(arg, 'utf8'));
			} else {
				proposal.args.push(arg);
			}
		}

		logger.debug(`${method} - chaincode ID:${proposal.chaincodeName}`);
		...
		proposal.header = buildHeader(idContext, channelHeader);
		...
		proposal.transactionId = idContext.transactionId;
	...
	signProposal(param = checkParameter('param')) {
		const method = `signProposal[${this.chaincodeName}]`;
		logger.debug(`${method} - start`);
		if (!this._endorsement || !this._endorsement.proposal) {
			throw Error('The endorsement proposal is not built');
		}
	...
	getSignedProposalEnvelope() {
		const method = `getSignedProposalEnvelope[${this.chaincodeName}]`;
		logger.debug(`${method} - start`);
		if (!this._endorsement || !this._endorsement.proposal) {
			throw Error('The endorsement proposal is not built');
		}
		...
		const signed_envelope = {
			signature: this._endorsement.signature,
			proposal_bytes: this._endorsement.proposal.toBuffer()
		};
	...
	async endorse(request) {
		const method = `endorse[${this.chaincodeName}]`;
		logger.debug(`${method} - start`);
	 ...
	 */
	buildCommit() {
		const method = `buildCommit[${this.chaincodeName}]`;
		logger.debug(`${method} - start`);
		...
		if (endorsements.length < 1) {
			logger.error(`${method} - no valid endorsements found`);
	...
	signCommit(param = checkParameter('param')) {
		const method = `signCommit[${this.chaincodeName}]`;
		logger.debug(`${method} - start`);
	...
	getSignedCommitEnvelope() {
		const method = `getSignedCommitEnvelope[${this.chaincodeName}]`;
		logger.debug(`${method} - start`);
	...
	async commit(request = {}) {
		const method = `commit[${this.chaincodeName}]`;
		logger.debug(`${method} - start`);
		...
		if (handler) {
			logger.debug(`${method} - calling the handler`);
			const result = await request.handler.commit(request, envelope);
			return result;
		} else if (targets) {
			logger.debug(`${method} - sending to the targets`);
	 ...
	 */
	buildQuery(idContext, request) {
		const method = `buildQuery[${this.chaincodeName}]`;
		logger.debug(`${method} - start`);
		this._query = {};
		this._query.type = 'query';
		this._buildProposal(idContext, request, this._query);
	...
	signQuery(param = checkParameter('param')) {
		const method = `signQuery[${this.chaincodeName}]`;
		logger.debug(`${method} - start`);
	...
	getSignedQueryEnvelope() {
		const method = `getSignedQueryEnvelope[${this.chaincodeName}]`;
		logger.debug(`${method} - start`);
		if (!this._query || !this._query.proposal) {
			throw Error('The query is not built');
		}
	 ...
	 */
	async query(request) {
		const method = `query[${this.chaincodeName}]`;
		logger.debug(`${method} - start`);
		const {handler, targets, request_timeout} = request;
		let results;
		...
		if (handler) {
			logger.debug(`${method} - Handler will send query`);
			results = await handler.query(request, signed_envelope);
		} else if (targets) {
			logger.debug(`${method} - Targets will be sent query`);
			const peers = this.channel.getTargetPeers(targets);
			const promises = peers.map(async (peer) => {
				return peer.sendProposal(signed_envelope, request_timeout);
			});
		...
		if (results) {
			results.forEach((result) => {
				if (result.isFulfilled()) {
					...
					logger.error(`${method} - Promise is rejected:${result.reason()}`);
			...
			this._query.proposalResponses.forEach((response) => {
				if (response.response && response.response.payload && response.response.status === 200) {
					logger.debug(`${method} - good status`);
					this._query.queryResults.push(response.response.payload);
				} else {
					logger.error(`${method} - unknown or missing results in query`);
	...
	async verifyProposalResponse(proposal_response) {
		...
		logger.debug(`${method} - found mspid ${mspid}`);
		...
		logger.debug(`${method} - found endorser's MSP`);
		...
		try {
			...
			logger.error(`${method} - getting endorser identity failed with: `, error);
		...
		try {
			...
			logger.debug(`${method} - have a valid identity`);
			...
			if (!identity.verify(digest, endorsement.signature)) {
				logger.error(`${method} - Proposal signature is not valid`);

				return false;
			}
		} catch (error) {
			logger.error(`${method} - verify failed with: `, error);

			return false;
		}

		logger.debug(`${method} - This endorsement has both a valid identity and valid signature`);
	...
	compareProposalResponseResults(proposal_responses) {
		...
		for (const i in proposal_responses) {
			const next_one = this._getProposalResponseResults(proposal_responses[i]);
			if (next_one.equals(first_one)) {
				logger.debug(`${method} - read/writes result sets match index=${i}`);
			} else {
				logger.error(`${method} - read/writes result sets do not match index=${i}`);
				return false;
			}
		}
