...
class Proposal extends ServiceAction {
	...
	build(idContext = checkParameter('idContext'), request) {
		...
		this._action.transactionId = idContext.newTransactionId();
		this._action.nonce = idContext.newNonce();
		...
		this._action.header = buildHeader(this._action.nonce, channelHeader);
