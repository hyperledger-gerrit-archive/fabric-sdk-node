...
var Member = class {
	 ...
	 */
	sendTransaction(proposalResponse, chaincodePropsoal) {
		...
		if (!chaincodePropsoal) {
			logger.error('Member.sendTransaction - input chaincodePropsoal missing');
			return Promise.reject(new Error('Missing chaincodePropsoal object parameter'));
		}
		...
		//logger.debug('Member.sendTransaction - chaincodePropsoal %j', chaincodePropsoal);
		...
		chaincodeActionPayload.setChaincodeProposalPayload(chaincodePropsoal.payload);

		var transactionAction = new _transProto.TransactionAction();
		transactionAction.setHeader(chaincodePropsoal.header);
	...
	sendDeploymentProposal(request) {
		...
		if(!request.peer) {
			logger.error('Invalid input parameter to "sendDeploymentProposal": must have "peer" object');
			return Promise.reject(new Error('Missing endorser peer object in the Deployment proposal request'));
		}
	...
	/**
	 * Sends a transaction proposal to an endorser.
	 *
	 * @param {Object} request:
	 *		target : {Object} Peer Object as the target of the request
	 *		chaincodeID : {String} The id of the chain code to perform transaction
	 *		args : {Array} Arguments for the funtion
	 * @returns Promise for a ProposalResponse
	 */
	sendTransactionProposal(request) {
		...
		// verify that the caller has included a peer object
		if(!request.peer) {
			logger.error('Missing endorser peer object in the Transaction proposal request');
			return Promise.reject(new Error('Missing endorser peer object in the Transaction proposal request'));
		}
	...
	/**
	 * Sends a query proposal to an endorser that will be handled by the chaincode.
	 * This request will be presented to the chaincode 'invoke' and must understand
	 * from the argument list that this a query request. The chaincode must also return
	 ...
	 * @returns Promise for a byte array
	 */
