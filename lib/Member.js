...
var Member = class {
	...
	sendTransaction(request) {
		...
		var payload_hash = this._chain.cryptoPrimitives.hash(chaincodeProposalPayloadNoTrans.getInput());
		chaincodeActionPayload.setChaincodeProposalPayload(payload_hash);
		...
		transactionAction.setHeader(header.toBuffer());
	...
	sendTransactionProposal(request) {
		...
		if (request && !request.args) {
			errorMsg = 'Missing arguments in Transaction proposal request';
		} else {
			errorMsg = Member._checkProposalRequest(request);
		}
	...
	static _buildHeader(creator, chain_id, chaincode_id, tx_id, nonce) {
		...
		if(chaincode_id) {
			...
			headerExt.setPayloadVisibility('a');
