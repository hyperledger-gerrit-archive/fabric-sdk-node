...
//		logger.debug('Member.sendTransaction - chaincodePropsoal %j', chaincodeProposal);
		...
		//chainHeader.setChainID('3939393');

		var signatureHeader = new _commonProto.SignatureHeader();
		signatureHeader.setCreator('somebody');
		signatureHeader.setNonce('12');
...
//		var envelope = new _commonProto.Envelope();
...
//		headerExt.setPayloadVisibility('????');
		...
		//cc_payload.setTransient(n/a); // application-level confidentiality related
...
function packageChaincode(chaincodePath, chaincodeId, dockerfileContents) {
	...
	if (dockerfileContents === undefined) {
		dockerfileContents = sdkUtils.getConfigSetting('dockerfile-contents',
