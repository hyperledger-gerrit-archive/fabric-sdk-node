...
var Member = class {
	 ...
	 */
	saveState() {
		logger.debug('Member.js -- saveState()');
	...
	restoreState() {
		...
		logger.debug('Member.js -- restoreState() for ' + self._keyValStoreName);
...
//		logger.debug('Member.sendTransaction - chaincodePropsoal %j', chaincodeProposal);
	...
	fromString(str) {
		...
		try{
			...
			throw new Error('ERROR on JSON.parse() : ' + err);
		}
