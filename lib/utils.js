...
module.exports.getLogger = function(name) {
	...
	if (process.env.HFC_LOGGING) {
		try {
			...
			logger.log('info', 'Successfully constructed a winston logger with configurations %j', config);
			saveLogger(logger);
			...
			saveLogger(logger);
			logger.log('warn', 'Failed to parse environment variable "HFC_LOGGING". Returned a winston logger with default configurations. Error: %s', err);
			return insertLoggerName(logger, name);
		}
	}
