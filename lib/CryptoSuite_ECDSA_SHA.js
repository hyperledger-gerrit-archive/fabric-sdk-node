...
var ECDSA_SHA = api.CryptoSuite.extend({
    ...
    asymmetricDecrypt: function(recipientPrivateKey, cipherText) {
        ...
        if (this._securityLevel != level) {
            throw Error('Invalid key. It\'s security does not match the current security level '
        ...
        if (ct_len <= Rb_len + D_len)
            throw new Error('Illegal cipherText length: ' + ct_len
                + ' must be > ' + (Rb_len + D_len));
    ...
    ecdsaPEMToPublicKey: function(chainKey) {
        ...
        var chainKeyHex = KEYUTIL.getHexFromPEM(pem, 'ECDSA PUBLIC KEY');
        // debug(chainKey);
        var certBuffer = utils.toArrayBuffer(new Buffer(chainKeyHex, 'hex'));
    ...
    _hkdf: function(ikm, keyBitLength, salt, info) {
        ...
        var key = this._hkdf2(sjcl_codec.bytes.toBits(new Buffer(ikm)), keyBitLength, sjcl_codec.bytes.toBits(salt), info, this._hashFunctionKeyDerivation);

        return sjcl_codec.bytes.fromBits(key);
    ...
    hmac: function(key, bytes) {
        ...
        var hmac = new sjcl.misc.hmac(sjcl_codec.bytes.toBits(key),
            this._hashFunctionKeyDerivation);
        hmac.update(sjcl_codec.bytes.toBits(bytes));
        var result = hmac.digest();
        debug('result: ', sjcl_codec.bytes.fromBits(result));
        return sjcl_codec.bytes.fromBits(result);
    },
    ...
    _checkSecurityLevel: function(securityLevel) {
        if (securityLevel != 256 && securityLevel != 384)
            throw new Error('Illegal level: ' + this._securityLevel
                + ' - must be either 256 or 384');
    },

    _checkHashFunction: function(hashAlgorithm) {
        if (!_isString(hashAlgorithm))
            throw new Error('Illegal Hash function family: ' + hashAlgorithm
                + ' - must be either SHA2 or SHA3');
    ...
    _hkdf2: function(ikm, keyBitLength, salt, info, Hash) {
        ...
        hmac.update(ikm);
        key = hmac.digest();
        // debug('prk: %j', new Buffer(sjcl_codec.bytes.fromBits(key)).toString('hex'));
        ...
        for (i = 1; i <= loops; i++) {
            ...
            hmac.update(sjcl_codec.bytes.toBits([i]));
