			...
			self.cryptoPrimitives.generateKey()
				...
				function (privateKey) {
					...
					try {
						...
						.then(
							function(csrPEM){							
								return resolve(new api.Enrollment(privateKey,new Buffer(csrPEM).toString('hex')));
							},
							function(err){
								return reject(err);
							}
				...
				response.on('end', function () {
					...
					try {
						...
						if (enrollResponse.success) {
							//we want the result field which is Base64-encoded
...
module.exports.FabricCOPServices = FabricCOPServices;
