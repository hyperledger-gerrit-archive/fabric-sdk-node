...
var CryptoSuite_ECDSA_AES = class extends api.CryptoSuite {
	...
	sign(key, digest, opts) {
		...
		// module './ecdsa/key.js'
		var signKey = this._ecdsa.keyFromPrivate(key._privateKey.prvKeyHex, 'hex');
	...
	verify(key, signature, digest) {
		var pubKey = this._ecdsa.keyFromPublic(key.getPublicKey().pubKeyHex, 'hex');
	...
	encrypt(key, plaintext, opts) {
		// Note that the statement below uses internal implementation specific to the
		// module './ecdsa/key.js'
		var key = this._ecdsa.keyFromPublic(key.getPublicKey().pubKeyHex, 'hex');

		// Generate ephemeral key-pair
		var ephKeyPair = KEYUTIL.generateKeypair('EC', this._curveName);
		var ephPrivKey = this._ecdsa.keyFromPrivate(ephKeyPair.prvKeyObj.prvKeyHex, 'hex');
		...
		var Z = ephPrivKey.derive(key.pub);
		logger.debug('[Z]: %j', Z);
		var kdfOutput = this._hkdf(Z.toArray(), ECIESKDFOutput, null, null);
