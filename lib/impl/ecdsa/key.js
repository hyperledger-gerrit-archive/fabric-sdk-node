...
module.exports = class {
	...
	constructor(key, keySize) {
		...
		this._privateKey = (typeof key === 'undefined') ? null : key;
	}
	...
	getSKI() {
		...
		sk.startSequence();
		sk.writeInt(1);
		sk.writeBuffer(new Buffer(this._privateKey.prvKeyHex, 'hex'), 4);
		sk.writeByte(160);
		sk.writeByte(7);
	...
	getPublicKey() {
		var f = new ECDSA({ curve: this._privateKey.curveName });
		f.setPublicKeyHex(this._privateKey.pubKeyHex);
		f.isPrivate = false;
		f.isPublic = true;
		return f;
	}
	...
	toBytes() {
		...
		return KEYUTIL.getPEM(this._privateKey, 'PKCS8PRV');
	}
};