...
var Chain = class {
	...
	sendTransactionProposal(request) {
		...
		.then(
			function(userContext) {
				let header = Chain._buildHeader(userContext.getEnrollment().certificate, request.chainId, request.chaincodeId, request.txId, request.nonce);
				let proposal = this._buildProposal(invokeSpec, header);
				let signed_proposal = this._signProposal(userContext.getEnrollment(), proposal);
	...
	sendTransaction(request) {
		...
		.then(
			function(userContext) {
				let sig = this.cryptoPrimitives.sign(userContext.getEnrollment().privateKey, payload_bytes);
