...
var CryptoSuite_PKCS11 = class extends api.CryptoSuite {
	...
	constructor(keySize, pkcs11Lib, pkcs11Pin) {
		...
		if (typeof pkcs11Pin === 'undefined' || pkcs11Pin === null ||
			...
			pkcs11Pin = null;
			logger.info(__func() +
				    'no PKCS11 PIN specified, keys are ephemeral only');
		}
	...
	_pkcs11OpenSession(pkcs11, pkcs11Lib, pkcs11Pin) {
		...
		try {
			...
			logger.debug(__func() + 'C_GetSlotInfo(' +
				     (slots.length-1).toString()+'): ' +
				     util.inspect(pkcs11.C_GetSlotInfo(slot),
						  {depth: null}));
			// Getting info about token
			logger.debug(__func() + 'C_GetTokenInfo(' +
				     (slots.length-1).toString()+'): ' +
	...
	_pkcs11SkiToHandle(pkcs11, pkcs11Session, ski) {
		try {
			 ...
			 */
			var secretKeyHandle = this._pkcs11FindObjects(
	...
	deriveKey(key, opts) {
		...
		if (typeof opts === 'undefined' || opts === null ||
			...
			throw new Error(__func() +
					'opts.pub must be PKCS11_ECDSA_KEY type public key');

		return this._pkcs11DeriveKey(this._pkcs11, this._pkcs11Session, key,
					     opts.pub);
	}
