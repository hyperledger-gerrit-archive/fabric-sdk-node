...
var __func = function() {
    ...
    return callsite()[1].getFunctionName() + '[' + callsite()[1].getLineNumber() + ']: ';
};

var CryptoSuite_PKCS11 = class extends api.CryptoSuite {

    /*
     * If pkcs11Lib is not specified or null, its value will be taken from the
     ...
     *
     * Similarly, if pkcs11Pin is not specified or null, its value will be taken
     * from the CRYPTO_PKCS11_PIN en var, and if the env var is not set, its
     * value will be taken from the crypto-pkcs11-pin key in the configuration
     * file.
     */
    constructor(keySize, pkcs11Lib, pkcs11Pin) {
	...
	/*
	 * If no pkcs11Lib specified, get it from env var or config file.
	 */
	if (typeof pkcs11Lib === 'undefined' || pkcs11Lib === null)
	    pkcs11Lib = utils.getConfigSetting('crypto-pkcs11-lib');
	if (typeof pkcs11Lib === 'undefined' || pkcs11Lib === null ||
	    typeof pkcs11Lib !== 'string')
	    throw new Error(__func() + 'PKCS11 library path must be set');
	/*
	 * If no pkcs11Pin specified, get it from env var or config file.
	 */
	...
	if (typeof pkcs11Pin === 'undefined' || pkcs11Pin === null ||
	    ...
	    pkcs11Pin = null;
	    logger.info(__func() + 'no PKCS11 PIN specified, keys are ephemeral only');
	}
	
	super();

	this._keySize = keySize;
	logger.info(__func() + ': keySize ' + this._keySize);

	/*
	 * Load native PKCS11 library, open PKCS11 session and login.
	 */
	this._pkcs11 = new pkcs11js.PKCS11();
	this._pkcs11OpenSession(this._pkcs11, pkcs11Lib, pkcs11Pin);
	/*
	...
	{
	    logger.info(__func() + 'workaround opencryptoki EC point wrong length: ' +
			ecpt.length);
     ...
     */
    _pkcs11OpenSession(pkcs11, pkcs11Lib, pkcs11Pin) {
	...
	try {
	    ...
	    logger.debug(__func() + 'C_GetSlotInfo(' + (slots.length-1).toString()+'): ' +
			 util.inspect(pkcs11.C_GetSlotInfo(slot), {depth: null}));
	    // Getting info about token
	    logger.debug(__func() + 'C_GetTokenInfo(' + (slots.length-1).toString()+'): ' +
			 util.inspect(pkcs11.C_GetTokenInfo(slot), {depth: null}));

	    // Getting info about Mechanism
	    logger.debug(__func() + 'C_GetMechanismList(' +
	    ...
	    logger.debug(__func() + 'C_GetSessionInfo(' +
			 ...
			 util.inspect(pkcs11.C_GetSessionInfo(this._pkcs11Session),
				      {depth: null}));

	    // Login with PIN if provided.
	    if (pkcs11Pin != null) {
		pkcs11.C_Login(this._pkcs11Session, 1/*pkcs11js.CKU_USER*/, pkcs11Pin);
		this._pkcs11Login = true;
		logger.info(__func() + 'session login successful');
	    }
	    else {
		this._pkcs11Login = false;
		logger.info(__func() + 'session without login');
	    }

	    /*
	    var tod = ('000000000000000' + (new Date).getTime().toString(16)).substr(-16);
	    pkcs11.C_SeedRandom(this._pkcs11Session, Buffer.from(tod, 'hex'));
	    logger.info(__func() + 'random number generator seeded');
	    */
	    ...
	    if (this._pkcs11Session != null) pkcs11.C_CloseSession(this._pkcs11Session);
	    pkcs11.C_Finalize();
	    throw(e);
	}
	finally {
	...
	try {
		...
		pkcs11Session, { mechanism: pkcs11js.CKM_AES_KEY_GEN },	secretKeyTemplate);
	    /*
		...
		this._pkcs11GetAttributeValue(pkcs11, pkcs11Session,
					      handle, objectTemplate), { depth: null }));
     ...
     */
    _pkcs11GenerateKeyPair(pkcs11, pkcs11Session, pkcs11Token) {
	var ski = this._ski();
	...
	try {
		...
		this._pkcs11GetAttributeValue(pkcs11, pkcs11Session,
					      handles.privateKey, objectTemplate),
		{ depth: null }));
	    logger.debug(__func() + 'publicKey: ' + util.inspect(
		this._pkcs11GetAttributeValue(pkcs11, pkcs11Session,
					      handles.publicKey, objectTemplate),
		{ depth: null }));
	    /*
	     ...
	     */
	    /*
	    this._pkcs11SetAttributeValue(pkcs11, pkcs11Session,
					  handles.publicKey,
					  [{ type: pkcs11js.CKA_ID, value: ski }]);
	    this._pkcs11SetAttributeValue(pkcs11, pkcs11Session,
					  handles.privateKey,
					  [{ type: pkcs11js.CKA_ID, value: ski }]);
	    logger.debug(__func() + 'pub  ski: ' +
			 util.inspect((this._pkcs11GetAttributeValue(
			     pkcs11, pkcs11Session, handles.publicKey,
			     [{ type: pkcs11js.CKA_ID }]))[0].value, { depth: null }));
	    logger.debug(__func() + 'priv ski: ' +
			 util.inspect((this._pkcs11GetAttributeValue(
	...
	try {
	    ...
	    if (secretKeyHandle.length == 1) return { secretKey: secretKeyHandle[0] };
	    /*
		...
		throw new Error(__func() + 'no key with SKI ' + ski.toString('hex') +
				' found');
	...
	try {
	    ...
	    pkcs11.C_SignInit(pkcs11Session, { mechanism: pkcs11js.CKM_ECDSA },
			      key._handle);
	    return pkcs11.C_Sign(pkcs11Session, digest, Buffer.alloc(this._keySize));
	}
	catch(e) {
	    throw(e);
	}
	...
	try {
	     ...
	     */
	    pkcs11.C_VerifyInit(pkcs11Session, { mechanism: pkcs11js.CKM_ECDSA },
				key._handle);
	    return pkcs11.C_Verify(pkcs11Session, digest, signature);
	}
	catch(e) {
	    /*
	...
	try {
		...
		iv,
		pkcs11.C_Encrypt(pkcs11Session,
				 plainText, Buffer.alloc((plainText.length+16) & (~15))),
	    ]);
	}
	catch(e) {
	    throw(e);
	...
	while (obj) {
		...
		this._pkcs11GetAttributeValue(pkcs11, pkcs11Session, obj, objectTemplate)));
	    objs.push(obj);
     ...
     * which also encapsulates the public key. It'll also save the private key in
     * the KeyValueStore unless opts.ephemeral is set to true.
     *
	...
	case 'AES':
		...
		try {
		    ...
		    /*
		     * Put key in the session cache and return promise of the key.
		     */
	...
	case 'ECDSA':
	    return new Promise(function(resolve, reject) {
		try {
		    var attr = self._pkcs11GenerateKeyPair(
			self._pkcs11, self._pkcs11Session, token);
		    /*
		     * Put key in the session cache and return promise of the key.
		     */
	...
	default:
		...
		Error(__func() + 'must specify AES or ECDSA key algorithm'));
	}
    }
	...
	if (hit !== undefined) {
	    logger.debug(__func() + 'cache hit ' + util.inspect(hit, { depth: null }));
	...
	return new Promise(function(resolve, reject) {
	    try {
		var handle = self._pkcs11SkiToHandle(self._pkcs11,
						     self._pkcs11Session, ski);
		/*
		 * AES key.
		 */
		var key;
		if (typeof handle.secretKey !== 'undefined') {
		    if (self._keySize != 256) {
			throw new Error(__func() + 'key size mismatch, class: ' +
					self._keySize + ', ski: 256');
		    }
		    key = new aesKey({ ski: ski, key: handle.secretKey }, self._keySize);
		}
		/*
		 * ECDSA key.
		 */
		else {
		    var attr = self._pkcs11QueryEcparamsEcpt(self._pkcs11,
							     self._pkcs11Session,
							     handle.publicKey);
		
		    var keySize = _pkcs11ParamsOidToSize[
			attr.ecparams.toString('hex').toUpperCase()];
		    if (keySize === undefined || keySize != self._keySize) {
			throw new Error(__func() + 'key size mismatch, class: ' +
					self._keySize + ', ski: ' + keySize);
		    }
	...
	if (typeof key === 'undefined' || key === null || !(key instanceof ecdsaKey))
	...
	if (typeof key === 'undefined' || key === null ||
	    ...
	    throw new Error(__func() + 'key must be PKCS11_ECDSA_KEY type private key');
	if (typeof opts === 'undefined' || opts === null ||
	    typeof opts.pub === 'undefined' || opts.pub === null ||
	    !(opts.pub instanceof ecdsaKey) || opts.pub.isPrivate())
	    throw new Error(__func() + 'opts.pub must be PKCS11_ECDSA_KEY type public key');

	return this._pkcs11DeriveKey(this._pkcs11, this._pkcs11Session, key, opts.pub);
    }
	...
	case 'AES':
		...
		try {
			...
			self._pkcs11, self._pkcs11Session, raw, token);
		    /*
		     * Put key in the session cache and return promise of the key.
		     */
