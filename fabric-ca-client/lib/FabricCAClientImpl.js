...
var FabricCAServices = class extends BaseClient {
	...
	reenroll(currentUser) {
		...
		var subject = null;
		try {
			subject = getSubjectString(FabricCAServices.normalizeX509(cert));
		} catch(err) {
...
function getSubjectString(pem) {
	var hex = x509.pemToHex(pem);
	var d = ASN1HEX.getDecendantHexTLVByNthList(hex, 0, [0, 5]);
	var subject = x509.hex2dn(d); // format: '/C=US/ST=California/L=San Francisco/CN=Admin@org1.example.com'
