...
describe('FabricCAServices', () => {
	...
	const normalizeX509 = sinon.stub();
	...
	beforeEach(() => {
		...
		revert.push(FabricCAServicesRewire.__set__('normalizeX509', normalizeX509));
	});
	...
	describe('#constructor', () => {
		...
		it('should create a new FabricCAClient and store within the FabricCAServices object', () => {
			...
			//sinon.assert.calledWithNew(fabricCAClientStub);
		});
	});
	...
	describe('#enroll', () => {
		...
		it('should reject on enroll failure', async () => {
			...
			await service.enroll(req, atts).should.be.rejectedWith(/enroll error/);
		});
		...
		it('should reject in generate CSR failure', async () => {
			...
			await service.enroll(req, atts).should.be.rejectedWith(/Failed to generate CSR for enrollmemnt due to error \[Error: CSR error\]/);
		});
		...
		it('should reject in generate key failure', async () => {
			...
			await service.enroll(req, atts).should.be.rejectedWith(/Failed to generate key for enrollment due to error \[Error: Key error\]/);
		});
		...
		it('should return a known object on success', async () => {
			...
			const result = await service.enroll(req, atts);
	...
	describe('#reenroll', () => {
		...
		it('should return the correct object on success', async () => {
			...
			const atts = [{ name: 'penguin' }, { name: 'power' }];
			const result = await service.reenroll(user, atts);
