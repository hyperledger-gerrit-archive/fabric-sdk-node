...
if [[ "$CURRENT_TAG" = *"skip"* ]]; then
     ...
     if [[ $UNSTABLE_VER = "" ]]; then
        ...
        echo "\033[32m======> UNSTABLE_INCREMENT:" $UNSTABLE_INCREMENT "\033[0m"
     fi
     ...
     echo "\033[32m======> UNSTABLE_INCREMENT_VERSION:" $UNSTABLE_INCREMENT_VERSION "\033[0m"
     ...
     # Publish node modules on the current tag
