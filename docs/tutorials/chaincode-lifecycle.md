...
The installation, updating, and the starting of chaincode has had been
...
* {@link Channel#approveChaincodeForOrg approveChaincodeForOrg} - approve
the chaincode for an organization.
* {@link Channel#commitChaincode commitChaincode} - commit
...
// set the sequence (modification) number - required if not "1"
mychaincode.setSequence(1); //set for each new definition change
```
...
later when the chaincode is approved and committed. The chaincode instance that
performs the install will also store the package ID internally for the next step.
...
shows how they would by skip the install and just assign the
package ID value to the chaincode instance. The package ID value must be
the value that was returned by the peer when the chaincode was installed.

```
// set package ID value instead of doing an install
...
Each organizational must endorse and commit the approve transaction for
...
The transactions must be the exact same chaincode definition.
The definition does not include the package, it includes the package ID value
...
transactions without having installed the package, but has received the package ID
...
This step will commit a chaincode for user on your channel.
The committing action will
not change the chaincode or it's settings, but rather confirm the
approved organizational chaincode definitions for the channel. 
...
The action to actually count the votes must be endorsed by enough members
...
definition must be defined as "initRequired=true" for the "Init" method
to be called on this invoke transaction rather than the "Invoke" method
of your chaincode.

TODO need a much better description here
```
