This tutorial describes how to use yoru application to install a chaincode on
your peers, define a chaincode on a channel, and start using the chaincode to
submit transactions. This tutorial uses the Fabric chaincode lifecycle that was
introduced as an Alpha feature in Fabric version 2.0 and the fabric-client 2.0.
The api's for using the previous chaincode lifecycle will still be available
in fabric-client, but will not be discussed in this tutorial.
...
### Overview
The new Fabric chaincode lifecycle is a process that allows multiple
organizations to agree on the parameters of a chaincode by defining the
chaincode on the channel before it can be used to interact with the ledger. You
will need to enable the new Fabric chaincode lifecycle on your network by
setting the channel capabilities to `V2_0` to use the steps in this tutorial.

Channel members need to complete the following steps before they can start
using a chaincode:
* [Setup](#setup) - create the necessary application objects
* [Package](#package) - create a chaincode package from your source code
* [Install](#install) - install the chaincode package on your peers
* [Approve a definition for organization](#approve) - each organization needs to
  approve a chaincode definition in order to use the chaincode
* [Commit the definition to a channel](#commit) - After a sufficient number of
...
chaincode definition for your organization and commit a definition to a
channel.
...
chaincode to the target peers in your organization. This request will need to
...
definition for their organization. The organizational chaincode definition
transaction may be submitted at any time, but must be submitted before the
...
one organization can commit the definition to the channel. In order to for the
...
following scenarios:
- [New chaincode](#new)
- [Upgrading a chaincode](#upgrade)
- [Modify an endorsement policy](#modify)
- [Join a channel with a running chaincode](#join)
