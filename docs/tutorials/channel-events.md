...
The Fabric peer channel-based event service also allows for all users to be able
to receive the block events by sending a filtered block the contains minimal and
non secure information instead of the entire block that a listener
must have read access to receive.
...
registration.
* `connect(full_block)` - To have the client channel event hub connect with the fabric
network channel-based event service. This call must be made before events will be
received by your instance of a ChannelEventHub. When the channel event hub
connects with the it will request to receive blocks or filtered blocks.
...
peer's channel-based event service to receive full blocks. Since most non
admin users will not be able to see full blocks, connecting to
...
have the chaincode event payload information, only the chaincode event
name. If the payload information is required, the user must have access to the
full block and the channel event hub must be `conneted(true)` to receive the
