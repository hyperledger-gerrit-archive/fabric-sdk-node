...
to perform multiple queries or search through blocks. Note that due to the
timing of when blocks are added to the ledger and when a registration is made,
the peer's channel-based event service will send the latest block on the ledger
when the listener does not request a starting block. This block may be from a
transaction previous to the transaction being monitored.
