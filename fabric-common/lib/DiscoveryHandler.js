...
const {randomize, checkParameter, getLogger, getConfigSetting} = require('./Utils.js');
...
class DiscoveryHandler extends ServiceHandler {
	...
	async query(signed_envelope = checkParameter('signed_envelope'), request = {}) {
		...
		const peers = this.discovery.channel.getPeers(mspid);
	...
	/**
	 * This will submit transactions to be committed to one orderer at time from a provided
	...
	async commit(signed_envelope = checkParameter('signed_envelope'), request = {}) {
		...
		if (orderers && orderers.length > 0) {
			...
			for (const orderer of orderers) {
				logger.debug('%s - sending to orderer %s', method, orderer.name);
				try {
	...
	/**
	 * This method will submit transactions to be endorsed to peers as
	 * determined by the peer's discovery service
	 * @param {*} signed_proposal
	 * @param {*} request
	 */
	...
	async _endorse(endorsement_plan, request, proposal, timeout) {
		...
		if (endorsement_plan) {
			...
			const results = {};
			results.endorsements = null; // only from one layout
