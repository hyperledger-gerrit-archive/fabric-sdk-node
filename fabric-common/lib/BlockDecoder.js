...
function decodePrivateData(private_data) {
	if (!private_data) {
		return {};
	}
	const private_data_map = {};
	// map key is the sequence number of the transaction
	for (const txIndex in private_data.map) {
		const txPvtReadWriteSet = {};
		const txPvtReadWriteSet_proto = private_data.map[txIndex].value;
		txPvtReadWriteSet.data_model = txPvtReadWriteSet_proto.getDataModel();
