...
describe('Discovery', () => {
	...
	beforeEach(async () => {
		...
		client.getPeer = getPeer;
		getPeer.returns(peer);
		peer.connect.resolves(true);
	});
	...
	describe('#send', () => {
		...
		it('handle results from config if peer exist', async () => {
			discovery.build(idx);
			discovery.sign(idx);
			peer.name = 'host.com:1000';
		...
		it('handle results with chaincode query res', async () => {
			...
			discovery.sign(idx);
			peer.name = 'peer4';
			peer.connect = sinon.stub().throws(new Error('bad connect'));
	...
	describe('#_buildUrl', () => {
		...
		it('should handle current target as', () => {
			discovery._current_target = peer;
	...
	describe('#_buildPeer', () => {
		...
		it('should handle found peer on the channel', async () => {
			peer.name = 'mypeer';
			channel.addPeer(peer);
			const results = await discovery._buildPeer({endpoint: 'mypeer'});
			should.equal(results, peer);
		});
		it('should run', async () => {
			discovery.discoveryResults = {};
			discovery.discoveryResults.msps = {msp1: {
		...
		it('should handle peer not connect', async () => {
			discovery.discoveryResults = {};
			discovery.discoveryResults.msps = {msp1: {
				id: 'msp1',
				name: 'msp1',
			...
			peer.name = 'host3.com:1000';
			peer.connect.throws(new Error('failed to connect'));
	...
	describe('#_processPeers', () => {
		it('should handle missing peer state info', async () => {
			const q_peers = [
				{
					identity: TestUtils.createSerializedIdentity(),
					membership_info: {payload: TestUtils.createMembership()}
	...
	describe('#_processMembership', () => {
		it('should handle missing peer by org', async () => {
