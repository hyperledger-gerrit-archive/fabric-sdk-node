...
describe('Proposal', () => {
	...
	describe('#send', () => {
		...
		it('returns no results', async () => {
			proposal.build(idx);
			proposal.sign(idx);
			sinon.stub(peer, 'sendProposal').resolves({});
			const results = await proposal.send({targets: [peer]});
		...
		it('should be able to handle result error', async () => {
			proposal.build(idx);
			proposal.sign(idx);
			sinon.stub(peer, 'sendProposal').resolves(new Error('forced resolved error'));
			const results = await proposal.send({targets: [peer]});
			should.exist(results.errors);
			if (results.errors && results.errors[0]) {
		...
		it('should be able to handle rejected error', async () => {
			proposal.build(idx);
			proposal.sign(idx);
			sinon.stub(peer, 'sendProposal').rejects(new Error('forced rejected error'));
			const results = await proposal.send({targets: [peer]});
			should.exist(results.errors);
			if (results.errors && results.errors[0]) {
		...
		it('should have responses when status included', async () => {
			proposal.build(idx);
			proposal.sign(idx);
			sinon.stub(peer, 'sendProposal').resolves({response: {status: 200}});
			const results = await proposal.send({targets: [peer]});
		...
		it('should have queryResults when this is a query', async () => {
			...
			proposal.sign(idx);
			proposal.type = 'Query';
			sinon.stub(peer, 'sendProposal').resolves({response: {status: 400, payload: 'query payload'}});
			const results = await proposal.send({targets: [peer]});
		...
		it('should have unknown queryResults when this is a query and unknown responses', async () => {
			...
			proposal.sign(idx);
			proposal.type = 'Query';
			sinon.stub(peer, 'sendProposal').resolves({response: {payload: 'query payload'}});
			const results = await proposal.send({targets: [peer]});
			should.not.exist(results.queryResults);
		});
