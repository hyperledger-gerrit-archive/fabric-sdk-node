...
describe('DiscoveryHandler', () => {
	...
	*/
	...
	describe('#query', () => {
		...
		it('should reject if peers are missing', async () => {
			await discoveryHandler.query('signed_envelope', {mspid: 'msp3'}).should.be.rejectedWith(/No peers assigned to the channel/);
		});
		it('should run with peers assigned by mspid', async () => {
		...
		it('should run with peers assigned', async () => {
		...
		it('should run with peers assigned', async () => {
			...
			channel.getPeer(org1[1]).sendProposal = sandbox.stub().rejects(error);
	...
	describe('#commit', () => {
		...
		it('should reject if orderers are missing', async () => {
			await discoveryHandler.commit('signed_envelope', {mspid: 'msp3'}).should.be.rejectedWith(/No orderers assigned to the channel/);
		});
		...
		it('should reject orderer returns missing results', async () => {
			channel.removeOrderer(channel.getOrderer('orderer2'));
			channel.removeOrderer(channel.getOrderer('orderer3'));
			channel.getOrderer('orderer1').sendBroadcast = sandbox.stub().resolves();
		...
		it('should reject when status is not correct', async () => {
			channel.getOrderer('orderer2').sendBroadcast = sandbox.stub().resolves({status: 'FAILED'});
			await discoveryHandler.commit('signed_envelope', {mspid: 'msp2'})
				.should.be.rejectedWith(/Failed to send transaction successfully to the orderer status:FAILED/);
		});
		it('should reject when orderer returns an error', async () => {
			channel.getOrderer('orderer2').sendBroadcast = sandbox.stub().rejects(new Error('FAILED with Error'));
			await discoveryHandler.commit('signed_envelope', {mspid: 'msp2'})
	...
	describe('#_build_endorse_group_member', () => {
		...
		it('should run ok and return error when peer rejects', async () => {
			endorsement_plan.endorsements = {};
			channel.getPeer(org1[1]).sendProposal = sandbox.stub().rejects(Error('FAILED'));
			channel.getPeer(org1[2]).sendProposal = sandbox.stub().rejects(Error('FAILED'));
			// TEST CALL
			const results = await discoveryHandler._build_endorse_group_member(
				endorsement_plan, // endorsement plan
				endorsement_plan.groups.G0, // group
			...
			sinon.assert.called(channel.getPeer(org1[1]).sendProposal);
			sinon.assert.called(channel.getPeer(org1[2]).sendProposal);
			sinon.assert.calledWith(FakeLogger.error, '%s - error on endorsement to %s error %s');
		});
		it('should run ok when peer failed on last layout', async () => {
			endorsement_plan.endorsements = {};
			...
			results.response.status.should.equal(200);
			sinon.assert.notCalled(channel.getPeer(org1[1]).sendProposal);
			sinon.assert.called(channel.getPeer(org1[2]).sendProposal);
		...
		it('should run ok when peer failed on last layout', async () => {
			...
			results.response.status.should.equal(200);
			sinon.assert.notCalled(channel.getPeer(org1[1]).sendProposal);
			sinon.assert.notCalled(channel.getPeer(org1[2]).sendProposal);
			sinon.assert.calledWith(FakeLogger.debug, '%s - existing peer %s endorsement will be used');
		});
		it('should run ok when peer failed on last layout', async () => {
			endorsement_plan.endorsements = {};
		...
		it('should run ok when no endorsement found', async () => {
			endorsement_plan.endorsements = {};
			channel.removePeer(channel.getPeer(org1[1]));
	...
	describe('#_removePeers', () => {
		it('should ignore peers and orgs', async () => {
			// TEST CALL
			const results = discoveryHandler._removePeers(
				discoveryHandler._create_map([org1[1], org2[1]]), // ignored_peers
				discoveryHandler._create_map([org3[0]]), // ignored_orgs
				new Map(), // required_peers
		...
		it('should require peers and orgs', async () => {
			// TEST CALL
			const results = discoveryHandler._removePeers(
				new Map(), // ignored_peers
				new Map(), // ignored_orgs
				discoveryHandler._create_map([org3[2]]), // required_peers
	...
	describe('#_splitList', () => {
		it('should run all', async () => {
			...
			const preferred_peers = discoveryHandler._create_map([
				'peer10'
			]);
	...
	describe('#_getRandom', () => {
		it('should run with peer list', async () => {
			...
			for (const item of start_list) {
				...
				should.equal(found, true, `Peer ${item.name} was not found`);
			}
		});
