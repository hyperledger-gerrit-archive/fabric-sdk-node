...
class EventHubManager {
	...
	log() {
		...
		const connectedFactoryEventHubs = factoryEventHubs.filter(eh => eh.isconnected());
		...
		logger.debug(`Replay Event Hubs: ${this.newEventHubs.length}/${connectedReplayEventHubs.length}`);
	}
}
