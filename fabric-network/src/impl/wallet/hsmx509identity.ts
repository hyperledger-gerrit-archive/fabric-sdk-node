...
export class HsmX509Provider implements IdentityProvider {
	...
	public async setUserContext(client: Client, identity: HsmX509Identity, name: string): Promise<void> {
		...
		const publicKey = (cryptoSuite as any).createKeyFromRaw(identity.credentials.certificate);
		...
		// const privateKeyObj = await this.cryptoSuite.importKey(keyBuffer as any);
