...
describe('Contract', () => {
	...
	describe('#addContractListener', () => {
		...
		it('should change options.replay=undefined to options.replay=false', async () => {
			...
			sinon.assert.calledWith(contract.getCheckpointer, {chaincodeId, replay: false, checkpointer: sinon.match.instanceOf(BaseCheckpointer)});
		});
		...
		it('should change options.replay=\'true\' to options.replay=true', async () => {
			...
			sinon.assert.calledWith(contract.getCheckpointer, {chaincodeId, checkpointer: sinon.match.instanceOf(BaseCheckpointer), replay: true});
		});
	});
});
