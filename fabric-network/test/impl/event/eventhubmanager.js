...
describe('EventHubManager', () => {
	...
	beforeEach(() => {
		...
		eventHubSelectionStrategyStub = sandbox.createStubInstance(BaseEventHubSelectionStrategy);
		...
		defaultNewEventHub = sandbox.createStubInstance(ChannelEventHub);
	...
	describe('#getReplayEventHubs', () => {
		it('should return a an event hub from the new list', () => {
			const newEventHub = sandbox.createStubInstance(ChannelEventHub);
			newEventHub._chaincodeRegistrants = {};
			newEventHub._blockRegistrations = {};
			newEventHub._transactionRegistrations = {};
			...
			const peer = sandbox.createStubInstance(Peer);
			peer.getName.returns('peer1');
			const eventHub = eventHubManager.getReplayEventHubs([peer]);
			expect(eventHub).to.deep.equal([newEventHub]);
			expect(eventHubManager.newEventHubs).to.have.length(1);
		});

		it('should return a new event hub', () => {
			const newEventHub = sandbox.createStubInstance(ChannelEventHub);
			newEventHub._chaincodeRegistrants = {};
			newEventHub._blockRegistrations = {};
			newEventHub._transactionRegistrations = {};
			...
			eventHubManager.newEventHubs = [newEventHub];
			const peer = sandbox.createStubInstance(Peer);
			peer.getName.returns('peer2');
			const eventHub = eventHubManager.getReplayEventHubs([peer]);
			expect(eventHub).to.deep.equal([defaultNewEventHub]);
			expect(eventHubManager.newEventHubs).to.have.length(2);
		});
	});
