...
describe.only('BlockEventListener', () => {
	let sandbox;
	let channelStub;
	let contractStub;
	let networkStub;
	...
	beforeEach(() => {
		...
		blockEventListener = new BlockEventListener(networkStub, 'test', () => {}, {});
	});
	describe('#register', () => {
	...
	describe('#_onEvent', () => {
		...
		it('should save a checkpoint', () => {
			...
			blockEventListener._onEvent(block);
			sinon.assert.calledWith(checkpointerStub.save, 'mychannel', 'test', null, 10);
		});
