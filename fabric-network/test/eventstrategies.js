...
describe('EventStrategies', () => {
	...
	it('CHANNEL_SCOPE_ALLFORTX', () => {
		const result = EventStrategies.CHANNEL_SCOPE_ALLFORTX(stubEventHubFactory, stubNetwork, mspId);
		result.should.be.an.instanceOf(AllForTxStrategy);
	});

	it('CHANNEL_SCOPE_ANYFORTX', () => {
		const result = EventStrategies.CHANNEL_SCOPE_ANYFORTX(stubEventHubFactory, stubNetwork, mspId);
		result.should.be.an.instanceOf(AnyForTxStrategy);
	});
});
