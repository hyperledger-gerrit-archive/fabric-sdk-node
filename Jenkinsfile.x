...
node ('hyp-x') { // trigger build on x86_64 node test
  timestamps {
    try {
     ...
     withEnv([PATH=$GOPATH/bin:/usr/local/bin:/usr/bin:/usr/local/sbin:/usr/sbin:~/npm/bin:/home/jenkins/.nvm/versions/node/v${NODE_VER}/bin:$PATH']) {
     ...
     echo "------> Don't publish API Docs from verify job"
           ...
           if (env.GERRIT_EVENT_TYPE == 'patchset-created') {
              if (currentBuild.result == 'FAILURE') { // Other values: SUCCESS, UNSTABLE
               ...
               rocketSend channel: 'jenkins-robot', message: "Build Notification - STATUS: ${currentBuild.result} - BRANCH: ${env.GERRIT_BRANCH} - PROJECT: ${env.PROJECT} - (${env.BUILD_URL})"
              }
           }
       }
  } // timestamps block
