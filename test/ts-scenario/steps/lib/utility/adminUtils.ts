...
export async function getSubmitter(client: Client, peerAdmin: boolean, org: string, ccp: CommonConnectionProfile): Promise<Client.User> {
	if (peerAdmin) {
 ...
 */
async function getOrgAdminUser(client: Client, userOrg: string, ccp: CommonConnectionProfile): Promise<Client.User> {
	try {
 ...
 */
async function getMember(username: string, password: string, client: Client, userOrg: string, ccp: CommonConnectionProfile): Promise<Client.User> {
...
export function getPeerObjectsForClientOnChannel(orgClient: Client, channelName: string, ccp: CommonConnectionProfile): Client.Peer[] {
 ...
 */
export async function assignOrgAdmin(client: Client, orgName: string, ccp: CommonConnectionProfile): Promise<void> {
 ...
 */
export async function isOrgChaincodeInstalled(orgName: string, ccp: CommonConnectionProfile, chaincodeName: string, chaincodeVersion: string): Promise<boolean> {
...
export async function isOrgChaincodeLifecycleInstalledOnChannel(orgName: string, ccp: CommonConnectionProfile, chaincodeName: string, deployedAs: string, channelName: string): Promise<boolean> {
...
export async function isOrgChaincodeLifecycleCommittedOnChannel(orgName: string, ccp: CommonConnectionProfile, chaincodeName: string, deployedAs: string, channelName: string): Promise<boolean> {
 ...
 * @param chaincodeVersion the version of the contract
 */
export async function isChaincodeInstantiatedOnChannel(orgName: string, ccp: CommonConnectionProfile, channelName: string, chaincodeName: string, chaincodeVersion: string): Promise<boolean> {
...
export async function performChannelQueryOperation(queryOperation: string, channelName: string, orgName: string, ccp: CommonConnectionProfile, args: any): Promise<any> {
	const clientPath: string = path.join(__dirname, Constants.UTIL_TO_CONFIG, orgName + '.json');
	const orgClient: Client = await Client.loadFromConfig(clientPath);
 ...
 */
export async function isOrgChannelJoined(orgName: string, ccp: CommonConnectionProfile, channelName: string): Promise<boolean> {
	const clientPath: string = path.join(__dirname, Constants.UTIL_TO_CONFIG, orgName + '.json');
	const orgClient: Client = await Client.loadFromConfig(clientPath);
