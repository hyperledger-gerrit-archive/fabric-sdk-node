...
// Policies to use
const ENDORSEMENT_POLICY_1OF_ANY: string = '"OR (\'Org1MSP.member\',\'Org2MSP.member\')"';
const ENDORSEMENT_POLICY_2OF_ANY: string = '"AND (\'Org1MSP.member\',\'Org2MSP.member\')"';
...
export async function cli_lifecycle_chaincode_commit(ccReference: string, ccVersion: string, orgName: string, channelName: string, ccp: CommonConnectionProfile, sequence: string, tls: boolean): Promise<void> {
	try {
		...
		// --peerAddresses
		const peerAddresses: string = 'peer0.org1.example.com:7051 peer0.org2.example.com:8051';
		//  --tlsRootCertFiles
		const tlsCerts: string = '/etc/hyperledger/config/crypto-config/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt /etc/hyperledger/config/crypto-config/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt';
