...
const INSTANTIATE_TIMEOUT = BaseUtils.getTimeoutDuration(Constants.STEP_SHORT);
...
export async function cli_chaincode_instantiate(ccType: string, ccName: string, ccVersion: string, initArgs: string, channelName: string, policy: string, tls: boolean) {
	try {
		...
		const timeoutId = setTimeout(() => { throw new Error(`instantiate smart contract ${ccName} on channel ${channelName} exceeded the default timeout ${INSTANTIATE_TIMEOUT}ms`); }, INSTANTIATE_TIMEOUT);
		let deployed = false;
		while (!deployed) {
			...
			if (response.includes(`Name: ${ccName}, Version: ${ccVersion}`)) {
				...
				await BaseUtils.sleep(BaseUtils.getTimeoutDuration(Constants.INC_SHORT));
			}
		}
