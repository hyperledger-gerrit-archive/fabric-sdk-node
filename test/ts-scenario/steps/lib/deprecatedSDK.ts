...
export async function deprecated_sdk_chaincode_install_for_org(ccType: string, ccName: string,  ccVersion: string, chaincodeId: string, tls: boolean, ccp: CommonConnectionProfile, orgName: string, channelName: string) {
	...
	try {
		...
		BaseUtils.logMsg(`Installing chaincode with ID ${chaincodeId} on organization ${orgName} peers [${ccp.getPeersForOrganization(orgName).toString()}] ...`, undefined);

		const results = await client.installChaincode(request as any);

		const proposalResponses = results[0];
		if (!proposalResponses) {
			throw new Error('No response returned from client.installChaincode() request');
		}
		...
		if (!proposalResponsesValid) {
			throw new Error(`Failed to send install Proposal or receive valid response: ${JSON.stringify(errors)}`);
		} else {
			BaseUtils.logMsg(`Successfully installed chaincode with ID [${ccName}]`, undefined);
			return await BaseUtils.sleep(BaseUtils.getTimeoutDuration(Constants.INC_SHORT));
		}
	} catch (err) {
		BaseUtils.logError('Failed to install chaincode', undefined);
		throw err;
	}
}
 ...
 */
export async function deprecated_sdk_chaincode_instantiate(ccName: string, ccType: string, ccVersion: string, chaincodeId: string, args: string, upgrade: boolean, tls: boolean, ccp: CommonConnectionProfile, orgName: string, channelName: string, policy: string) {
	if (!supportedLanguageTypes.includes(ccType)) {
		Promise.reject(`Unsupported test ccType: ${ccType}`);
	}
	...
	try {
		BaseUtils.logMsg(`Performing ${type} transaction on chaincode with ID [${chaincodeId}] as organization [${orgName}] ...`, undefined);
		...
		const proposalResponses = results[0];
		if (!proposalResponses) {
			throw new Error('No response returned from channel.sendInstantiateProposal() request');
		}
		...
		if (results && !(results[0] instanceof Error) && results[0].status === 'SUCCESS') {
			BaseUtils.logMsg(`Successfully performed ${type} transaction on chaincode with ID [${chaincodeId}]`, undefined);
			return await BaseUtils.sleep(BaseUtils.getTimeoutDuration(Constants.INC_SHORT));
		} else {
			const msg = `Failed to order the ${type} transaction. Error code: ${results[0].status}`;
			BaseUtils.logError(msg, undefined);
			throw new Error(msg);
		}
	} catch (err) {
		BaseUtils.logError('Failed to perform ' + type + ' instantiation on chaincode with ID [' + ccName + ']', undefined);
