...
export async function sdk_chaincode_install_for_org(ccType: 'golang' | 'car' | 'java' | 'node', ccName: string,  ccVersion: string, chaincodeId: string, tls: boolean, ccp: CommonConnectionProfile, orgName: string, channelName: string): Promise<void> {
 ...
 */
export async function sdk_chaincode_instantiate(ccName: string, ccType: 'golang' | 'car' | 'java' | 'node', ccVersion: string, chaincodeId: string, args: string, upgrade: boolean, tls: boolean, ccp: CommonConnectionProfile, orgName: string, channelName: string, policy: any): Promise<void> {
	if (!supportedLanguageTypes.includes(ccType)) {
		Promise.reject(`Unsupported test ccType: ${ccType}`);
	}
