...
const lifecycleCcp: CommonConnectionProfile = new CommonConnectionProfile(path.join(__dirname, '../config', 'ccp-lifecycle.json'), true);
const lifecycleCcpTls: CommonConnectionProfile = new CommonConnectionProfile(path.join(__dirname, '../config', 'ccp-lifecycle-tls.json'), true);
...
Given(/^I package a (node|java|golang) contract at version (.+?) named (.+?) as organizations (.+?) with initialization (required|unrequired)$/, { timeout: Constants.STEP_LONG as number }, async (contractType: string, contractVersion: string, contractName: string, orgs: string, initRequired: string) => {
	...
	const ccp: CommonConnectionProfile = tls ? lifecycleCcpTls : lifecycleCcp;
