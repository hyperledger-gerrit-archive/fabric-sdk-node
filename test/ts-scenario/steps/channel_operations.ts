...
When(/^I perform a (.+?) operation on channel (.+?) with (.+?) the response (includes|matches|mirrors) fields (.+?)$/, { timeout: Constants.HUGE_TIME as number }, async (queryOperation, channelName, orgName, compareType, expectedResponse) => {
	...
	// Perform query
	const response = await AdminUtils.performChannelQueryOperation(queryOperation, channelName, orgName, ccp, undefined);
...
When(/^I perform a (.+?) operation with arguments (.+?) on channel (.+?) with (.+?) the response (includes|matches|mirrors) fields (.+?)$/, { timeout: Constants.HUGE_TIME as number }, async (queryOperation, args, channelName, orgName, compareType, expectedResponse) => {
...
function validateResponse(compareType: string, response: any, expected: any, queryOperation: string) {
	// check response
...
function validateObjectKeyMatch(expected: any, actual: any, isMatch: boolean): any {
	...
	if (expected instanceof Object) {
		...
		//not an Object so "expected" is a value tht should (conditionally) match
		if (isMatch) {
			if (expected !== actual) {
