...
Given(/^I have a gateway named (.+?) with discovery set to (.+?) for user (.+?) using the connection profile named (.+?)$/, {timeout: BaseUtils.getTimeoutDuration(Constants.STEP_MED)}, async (gatewayName, useDiscovery, userName, ccpName) => {
...
When(/^I use the gateway named (.+?) to (.+?) a transaction with args (.+?) for contract (.+?) instantiated on channel (.+?)$/, {timeout: BaseUtils.getTimeoutDuration(Constants.STEP_MED)}, async (gatewayName, txnType, txnArgs, ccName, channelName) => {
	return await PerformGatewayTransaction(gatewayName, ccName, channelName, txnArgs, txnType);
});

Then(/^The gateway named (.+?) has a (.+?) type response$/, {timeout: BaseUtils.getTimeoutDuration(Constants.STEP_LONG)}, async (gatewayName, type) => {
...
Then(/^The gateway named (.+?) has a (.+?) type response matching (.+?)$/, {timeout: BaseUtils.getTimeoutDuration(Constants.STEP_LONG)}, async (gatewayName, type, expected) => {
