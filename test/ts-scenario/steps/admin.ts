...
import { deprecated_sdk_chaincode_install_for_org, deprecated_sdk_chaincode_instantiate } from './lib/deprecatedSDK';
...
Given(/^I use the cli to create and join the channel named (.+?) on the deployed network/, {timeout: BaseUtils.getTimeoutDuration(Constants.STEP_MED)}, async (channelName) => {
...
Given(/^I use the cli to update the channel with name (.+?) with config file (.+?) on the deployed network/, {timeout: BaseUtils.getTimeoutDuration(Constants.STEP_MED)}, async (channelName, configTxFile) => {
...
Given(/^I use the cli to deploy a (.+?) smart contract named (.+?) at version (.+?) for all organizations on channel (.+?) with endorsement policy (.+?) and arguments (.+?)$/, {timeout: BaseUtils.getTimeoutDuration(Constants.STEP_MED)}, async (ccType, ccName, ccVersion, channelName, policy, initArgs) => {
...
Given(/^I use the deprecated sdk to (.+?) a (.+?) smart contract named (.+?) at version (.+?) as (.+?) for all organizations on channel (.+?) with endorsement policy (.+?) and arguments (.+?) with the connection profile named (.+?)$/, {timeout: BaseUtils.getTimeoutDuration(Constants.STEP_MED)}, async (deployType, ccType, ccName, ccVersion, ccId, channelName, policyType, initArgs, ccpName) => {
	...
	try {
		...
		if (AdminUtils.isContractInstalled(persistName)) {
			...
			for (const orgName of Object.keys(ccp.getOrganizations())) {
				await deprecated_sdk_chaincode_install_for_org(ccType, ccName, ccVersion, ccId, tls, ccp, orgName, channelName);
			}
		...
		if (AdminUtils.isInstantiatedOnChannel(persistName, channelName)) {
			...
			await deprecated_sdk_chaincode_instantiate(ccName, ccType, ccVersion, ccId, initArgs, isUpgrade, tls, ccp, Constants.DEFAULT_ORG, channelName, policy);
