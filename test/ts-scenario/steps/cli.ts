...
const ccpNonTls: CommonConnectionProfile = new CommonConnectionProfile(path.join(__dirname, '../config', 'ccp.json'), true);
const ccpTls: CommonConnectionProfile = new CommonConnectionProfile(path.join(__dirname, '../config', 'ccp-tls.json'), true);
...
Given(/^I use the cli to create and join the channel named (.+?) on the deployed network/, { timeout: Constants.STEP_MED as number }, async (channelName: string) => {
	...
	const ccp: CommonConnectionProfile = tls ? ccpTls : ccpNonTls;
	...
	try {
		...
		for (const orgName of orgNames) {
			const alreadyJoint: boolean = await AdminUtils.isOrgChannelJoined(orgName, ccp, channelName);
			if (alreadyJoint) {
...
Given(/^I use the cli to lifecycle deploy a (.+?) smart contract named (.+?) at version (.+?) as (.+?) for all organizations on channel (.+?) with default endorsement policy and arguments (.+?)$/, { timeout: Constants.STEP_MED as number }, async (ccType: string, ccName: string, ccVersion: string, ccReference: string, channelName: string, initArgs: string) => {
	...
	try {
		...
		if (isCommitted) {
			...
			// Install on each org
			for (const orgName of orgNames) {
				const isInstalled: boolean = await AdminUtils.isOrgChaincodeLifecycleInstalledOnChannel(orgName, ccp, ccName, ccReference, channelName);
				if (isInstalled) {
