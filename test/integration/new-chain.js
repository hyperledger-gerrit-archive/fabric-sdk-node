...
//
test('\n\n** TEST ** new chain using chain.initializeChannel() method with good orderer address', function(t) {
	//
	...
	.then(
		function(data) {
			...
			// send to orderer
			return chain.initializeChannel(request);
		},
		function(err) {
	...
	.then(
		function(nothing) {
			...
			return chain.sendJoinChannelProposal(request);
		},
		function(err) {
	...
	.then(
		function(response) {
			console.log(' Join Chanel R E S P O N S E ::'+ JSON.stringify(response));
...
//
test('\n\n** TEST ** new chain - chain.initializeChannel() fail due to already exist', function(t) {
	//
	// Create and configure the test chain
	//
	var client = new hfc();
