...
test('\n\n** Test chaincode install using chaincodePath to create chaincodePackage **\n\n', (t) => {
	...
	installChaincode(params, t)
		...
		(err) => {
			...
			installChaincode(params, t)
				.then((info) => {
					if (info && info.response.status === 500 && info.response.message.toString().indexOf('install.'+params.chaincodeVersion+' exists') > -1) {
		...
		.then((data) => {
			...
			installChaincode(params, t)
				...
				(err) => {
					...
					installChaincode(params, t)
						.then((info) => {
							if (info && info.response.status === 500 && info.response.message.toString().indexOf('install-package.'+params.chaincodeVersion+' exists') > -1) {
