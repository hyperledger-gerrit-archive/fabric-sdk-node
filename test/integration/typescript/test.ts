...
test('use the connection profile file', (t) => {
	const config_network = path.resolve(__dirname, '../../fixtures/network3.yaml');
	const config_org1 = path.resolve(__dirname, '../../fixtures/org1.yaml');
	const config_org2 = path.resolve(__dirname, '../../fixtures/org2.yaml');
	const channel_name = 'mychannel3';
	...
	let fabca: FabricCAServices = null;
	let config = null;
	...
	client.initCredentialStores()
		.then(() => {
			...
			fabca = client.getCertificateAuthority();
