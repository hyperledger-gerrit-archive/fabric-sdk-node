...
test('\n\n ** FabricCAServices - CertificateService Test **\n\n', async (t) => {
	try {
		...
		// there should be two certificates for each admin at the beginning
		...
		t.equal(resp.result.certs.length, 1, 'there should be 1 certificates for the admin user');
		...
		resp = await certificateService1.getCertificates({aki: 'a22daf356b2aab5792ea53e35f66fccef1d7f1aa2b3a2b92dbfbf96a448ea26a'}, user1);
		t.equal(resp.success, true, 'certificate service should response success');
	} catch (e) {
		t.fail(e);
		t.end();
