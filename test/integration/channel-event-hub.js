...
test('Test chaincode instantiate with event, transaction invocation with chaincode event, and query number of chaincode events', (t) => {
	...
	Client.newDefaultKeyValueStore({
		...
		let event_monitor = new Promise((resolve, reject) => {
			...
			regid = eh.registerChaincodeEvent(chaincode_id.toString(), '^evtsender*', (event) => {
				clearTimeout(handle);
				eh.unregisterChaincodeEvent(regid);
				t.pass('Successfully received the chaincode event');
				resolve();
			}, (error)=> {
				clearTimeout(handle);
				eh.unregisterChaincodeEvent(regid);
		...
		//TODO check the results of both the event and send transaction
		...
		let event_monitor_1 =  new Promise((resolve, reject) => {
			...
			eh.registerTxEvent(req1.txId.getTransactionID(), (txnid, code) => {
				clearTimeout(handle);
				eh.unregisterTxEvent(txnid);
				t.pass('Event1 has transaction code:'+ code + ' for transaction id ::'+txnid);
