...
test('*****  Test channel events', async (t) => {
	...
	try {
		...
		let connecter = new Promise((resolve, reject) => {
			...
			event_hub.connect({full_block: false},(connected_hub) => {
				clearTimeout(handle);
				if (connected_hub.isconnected()) {
					t.pass('able to connect to the event service using a connect callback');
					resolve();
				} else {
		...
		try {
			await connecter;
			t.fail('Should have received the error callback error');
		} catch(error) {
			t.pass('Failed to connect to event service ::' + error.toString());
		}
