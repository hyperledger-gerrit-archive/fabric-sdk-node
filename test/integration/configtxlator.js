...
test('\n\n***** configtxlator flow for create and then update  *****\n\n', function(t) {
	...
	return Client.newDefaultKeyValueStore({
		...
		//we would like to just pass both of the config objects to the configtxlator tool directly
		// but that does not work, writing to file to try using the attach
		fs.writeFileSync(path.join(__dirname, '../fixtures/channel/original_config.proto'), original_config_proto);
		fs.writeFileSync(path.join(__dirname, '../fixtures/channel/updated_config.proto'), updated_config_proto);

		// showing where this call should be made... we just do not have it working yet
		return agent.post('http://127.0.0.1:7059/configtxlator/compute/update-from-configs')
			.field('channel', channel_name)
			.attach('original', path.join(__dirname, '../fixtures/channel/original_config.proto'))
			.attach('updated', path.join(__dirname, '../fixtures/channel/updated_config.proto'));
	}).then((response) =>{
		t.pass('-- had configtxlator compute the updated config object, but we will not use');
		config_proto = response.body;
		// since we are not able to get the compute to work when called above
		//  .. we will have to read from file the updated_config created by using 'curl' like so
		// curl -X POST --raw
		//      -F channel=mychannelator
		//      -F original=@original_config.proto
		//      -F updated=@updated_config.proto
		//      http://127.0.0.1:7059/configtxlator/compute/update-from-configs > updated_config_update.proto
		config_proto = fs.readFileSync(path.join(__dirname, '../fixtures/channel/updated_config_update.proto'));
		t.pass('Successfully read a good updated config proto from file');
