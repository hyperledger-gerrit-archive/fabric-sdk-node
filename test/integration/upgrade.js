...
test('\n\n **** Testing re-initializing states during upgrade ****', (t) => {
	...
	e2eUtils.installChaincode('org1', testUtil.CHAINCODE_UPGRADE_PATH_V2, 'v2', t)
	.then(() => {
		return e2eUtils.installChaincode('org2', testUtil.CHAINCODE_UPGRADE_PATH_V2, 'v2', t);
	}, (err) => {
		t.fail('Failed to install chaincode in peers of organization "org1". ' + err.stack ? err.stack : err);
		t.end();
	}).then(() => {
		return e2eUtils.instantiateChaincode('org1', testUtil.CHAINCODE_UPGRADE_PATH_V2, 'v2', true, t);
	}).then((results) => {

		logger.debug('Successfully upgraded chaincode to version v2');
		return 	e2eUtils.queryChaincode('org1', 'v2', '1000', t);
