	...
	hfc.newDefaultKeyValueStore({
		...
		if (!useSteps || steps.indexOf('step1') >= 0) {
			...
			promise = promise.then((admin) => {
				...
				request = eputil.createRequest(chaincode_id, chain_id, 'init', []);
		...
		if (!useSteps || steps.indexOf('step2') >= 0) {
			promise = promise.then((data) => {
				...
				request = eputil.createRequest(chaincode_id, chain_id, 'invoke', ['invoke', 'SEVERE']);
		...
		if (!useSteps || steps.indexOf('step3') >= 0) {
			promise = promise.then((data) => {
				...
				request = eputil.createRequest(chaincode_id, chain_id, 'invoke', ['query']);
		...
		if (!useSteps || steps.indexOf('step4') >= 0) {
			...
			// be invalid
			var req1 = eputil.createRequest(chaincode_id, chain_id, 'invoke', ['invoke', 'SEVERE']);
			var req2 = eputil.createRequest(chaincode_id, chain_id, 'invoke', ['invoke', 'SEVERE']);
			promise = promise.then((data) => {
			...
			(err) => {
				...
				if (steps.length === 1 && steps[0] === 'step4') {
					t.end();
				}
			},
			(err) => {
