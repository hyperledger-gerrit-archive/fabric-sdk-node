...
async function invoke(t, client, channel) {
	...
	try {
		...
		let results = await channel.sendTransactionProposal(request); //logged in as org2 user
...
async function manually(t, client) {
	try {
		...
		t.pass('Successfully got the config envelope');
