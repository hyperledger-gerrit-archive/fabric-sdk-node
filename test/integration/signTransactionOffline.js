...
test('Test sign a contract with a private key offline', async (t) => {
	try {
		...
		const { proposal, txId } = channel.generateTransactionProposal(transactionProposalReq, mspId, certPem);
		...
		const proposalResponses = await channel.sendSignedTransactionProposal(sendSignedProposalReq);
		...
		const commitProposal = await channel.generateCommitProposal(commitReq);
		...
		const response = await channel.sendSignedCommitProposal({
