...
test('Test sign a contract with a private key offline', async (t) => {
	try {
		...
		const { proposal, txId } = channel.generateEndorseProposal(transactionProposalReq, mspId, certPem);
		...
		const proposalResponses = await channel.sendSignedEndorseProposal(sendSignedProposalReq);
