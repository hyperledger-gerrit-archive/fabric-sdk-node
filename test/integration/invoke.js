...
test('\n\n***** E R R O R  T E S T I N G: invoke transaction with one endorsement *****\n\n', (t) => {
	...
	invokeChaincode('org2', 'v0', t, false/*useStore*/, 'ENDORSEMENT_POLICY_FAILURE', [peers[0]])
	.then((result) => {
		if(result){
			t.pass('Failed to invoke transaction chaincode on channel');

			t.comment('# Sending a proposal only to org2.');
			return invokeChaincode('org2', 'v0', t, false/*useStore*/, 'ENDORSEMENT_POLICY_FAILURE', [peers[1]]);
		}
		else {
...
test('\n\n***** invoke transaction with inverted order of endorsements *****\n\n', (t) => {
	...
	invokeChaincode('org2', 'v0', t, false/*useStore*/, false, [peers[1], peers[0]])
	.then((result) => {
...
function invokeChaincode(userOrg, version, t, useStore, shouldFail, peers){
