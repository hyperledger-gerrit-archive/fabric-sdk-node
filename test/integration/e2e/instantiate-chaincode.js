...
test('\n\n***** End-to-end flow: instantiate chaincode *****', (t) => {
	...
	.then((result) => {
		if(result){
			t.pass('Successfully instantiated chaincode in peers of organization "org1"');
			t.end();
		}
		else {
		...
		t.pass('** Successfully instantiated chaincode in peers of organization "org2"');
		t.end();
	}, (err) => {
		t.fail('Failed to instantiate chaincode in peers of organization "org2". ' + err.stack ? err.stack : err);
		t.end();
	}).catch((err) => {
