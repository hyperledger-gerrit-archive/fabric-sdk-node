...
test('\n\n***** Token end-to-end flow: double spending fails *****\n\n', async (t) => {
	...
	try {
		...
		const tokenId = '\x00tokenId\x00' + transferToken.id.tx_id + '\x00' + transferToken.id.index + '\x00';
		t.equals(
			err.message,
			'command response has error: input \'' + tokenId + '\' does not exist',
...
test('\n\n***** Token end-to-end flow: non owner transfer fails *****\n\n', async (t) => {
	try {
		...
		t.equals(
			err.message,
			'command response has error: the requestor does not own token',
