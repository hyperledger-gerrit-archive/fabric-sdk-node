...
test('\n\n***** use the connection profile file  *****\n\n', function (t) {
    ...
    client_org1.initCredentialStores()
        .then(() => {
            ...
            user.setEnrollment('user1');
            return ca1.register(user, admin);
        }).then((secret) => {
...
test('\n\n***** Enroll user and set user context using a specified caName *****\n\n', async function (t) {
    try {
        ...
        user1.setAffiliation(org_name);
        user1.setEnrollment(testuser);

        const secret = await ca1.register(user1, admin);
        ...
        let user2 = new User(testuser2);
        user2.setAffiliation(org_name);
        user2.setEnrollment(testuser2);
        const secret2 = await ca1.register(testuser2, admin);
...
test('\n\n***** Enroll user and set user context using a bad caName *****\n\n', async function (t) {
    try {
        ...
        user.setEnrollment(testuser);
        const secret = await ca1.register(user, admin);
