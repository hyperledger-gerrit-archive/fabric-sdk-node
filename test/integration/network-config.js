...
test('\n\n***** use the connection profile file  *****\n\n', async (t) => {
	 ...
	 */
		...
		const txPromise = new Promise((resolve, reject) => {
			...
			eventhub.registerTxEvent(query_tx_id,
				(tx, code, block_num) => {
					clearTimeout(handle);
					if (code !== 'VALID') {
		...
		const event_results = results[0]; // Promise all will return the results in order of the of Array
		const sendTransaction_results = results[1];
