...
async function getMember(username, password, client, userOrg, ccp) {
	...
	try {
		...
		if (!cryptoSuite) {
			...
			if (userOrg) {
				await cryptoSuite.setCryptoKeyStore(Client.newCryptoKeyStore({path: module.exports.storePathForOrg(org.name)}));
...
async function getOrgAdmin(client, userOrg, ccp) {
	try {
		...
		await cryptoSuite.setCryptoKeyStore(Client.newCryptoKeyStore({path: module.exports.storePathForOrg(userOrg)}));
		client.setCryptoSuite(cryptoSuite);
