...
async function performGatewayTransaction(gatewayName, ccName, channelName, args, submit) {
	...
	try {
		if (submit) {
			...
			gatewayObj.result = {type: 'submit', reponse: result.toString()};
			...
			gatewayObj.result = {type: 'evaluate', reponse: result.toString()};
...
function lastResponseCompare(gatewayName, msg) {
	const gatewayObj = gateways.get(gatewayName);
	return (gatewayObj.result.reponse.localeCompare(msg) === 0);
}
