...
module.exports = function () {
	...
	this.Then(/^I use the gateway named (.+?) to listen for block_events with listener (.+?) on chaincode (.+?) instantiated on channel (.+?)$/, {timeout: testUtil.TIMEOUTS.SHORT_STEP}, async (gatewayName, listenerName, ccName, channelName) => {
		return await network_util.createBlockListener(gatewayName, channelName, ccName, listenerName);
	});

	this.Then(/^I receive (.+?) events from the listener (.+?)$/, {timeout: testUtil.TIMEOUTS.SHORT_STEP}, async (calls, listenerName) => {
	...
	this.Then(/^I use the transaction named (.+?) to submit a transaction with args (.+?)$/, async (transactionName, args) => {
