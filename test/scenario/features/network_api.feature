	...
	Background:
 	...
 	Scenario: Using a Gateway I can submit and evaluate transactions on instantiated chaincode
		...
		And I can create a gateway named test_gateway as user User1 within Org1 using the tls common connection profile
	 	When I use the gateway named test_gateway to submit a transaction with args [initMarble,marble1,blue,35,tom] for chaincode marbles instantiated on channel mychannel
		Then The gateway named test_gateway has a submit type response
		When I use the gateway named test_gateway to evaluate transaction with args [readMarble,marble1] for chaincode marbles instantiated on channel mychannel
		Then The gateway named test_gateway has a evaluate type response matching {"color":"blue","docType":"marble","name":"marble1","owner":"tom","size":35}
