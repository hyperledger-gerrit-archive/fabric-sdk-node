...
Feature: Configure Fabric using SDK using discovery service and submit/evaluate using a network Gateway
	...
	Background:
 	...
 	Scenario: Using a Gateway I can submit and evaluate transactions on instantiated chaincode
		...
		And I can update channel with name mychannel with config file /Users/liamg/dev/fabric-sdk-node/test/scenario/config/crypto-config/mychannel-org1anchor.tx from the tls common connection profile
		...
		And I can create a gateway named test_gateway as user User1 within Org1 using the discovery common connection profile
