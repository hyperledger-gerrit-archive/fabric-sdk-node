...
Feature: Use the v2.0 chaincode lifecycle process
	...
	Scenario: Using the SDK I can run new chaincode
		...
		Then I can create and join a version_two capabilities channel named ourbank to two organizations
...
#		And I can install node chaincode at version 1.0.0 named example_cc to the tls Fabric network as organization Org2 on channel ourbank
#	 	And I can approve node chaincode at version 1.0.0 named example_cc to the tls Fabric network as organization Org1 on channel ourbank with endorsement policy 2ofAny
#	 	And I can approve node chaincode at version 1.0.0 named example_cc to the tls Fabric network as organization Org2 on channel ourbank with endorsement policy 2ofAny
#	 	And I can commit node chaincode at version 1.0.0 named example_cc to the tls Fabric network as organization Org1 on channel ourbank
#	 	And I can initialize node chaincode named example_cc to the tls Fabric network as organization Org1 on channel ourbank args [init,a,1000,b,2000]
#	 	And I can invoke node chaincode named example_cc to the tls Fabric network as organization Org2 on channel ourbank args [move,a,b,100]