...
Feature: Listen to block events using a contract
	...
	Scenario: Using a Contract I can listen to block events emmited by networks
		...
		When I use the gateway named test_gateway to listen for filtered_block_events with listener filteredBlockListener on chaincode events02 instantiated on channel mychannel
		...
		Then I receive 2 events from the listener filteredBlockListener
		When I use the gateway named test_gateway to listen for unfiltered_block_events with listener unfilteredBlockListener on chaincode events02 instantiated on channel mychannel
		When I use the gateway named test_gateway to submit a transaction with args [createValue] for chaincode events02 instantiated on channel mychannel
		Then I receive 2 events from the listener unfilteredBlockListener
