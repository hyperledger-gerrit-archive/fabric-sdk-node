...
Feature: Listen to events using a fabric-network
	...
	Scenario: Using a Contract I can listen to block events emitted by networks between a start and end block
		When I use the gateway named test_gateway to listen for unfiltered_block_events with listener unFilteredBlockListener between 0 and 2 on chaincode events01 instantiated on channel mychannel
		Then I receive at least 2 events from the listener unFilteredBlockListener
