 ...
 */
async function installChaincode(ccName, ccType, ccVersion, tls, ccp, orgName, channelName) {
	...
	try {
		...
		const ccPath = path.join(__dirname, '../chaincode', ccName, ccType);
		...
		console.log('Installing chaincode with ID [' + ccName + '] on ' + orgName + ' peers [' + ccp.getPeersForOrganization(orgName).toString() + ']');
		...
		if (!proposalResponsesValid) {
			...
			console.log('Successfully installed chaincode with ID [' + ccName + ']');
			return await testUtil.sleep(3000);
		}
	} catch (err) {
		console.error('Failed to install chaincode');
		throw err;
	}
}
 ...
 * @return {Promise} The return promise.
 */
async function instantiateChaincode(ccName, ccType, args, version, upgrade, tls, ccp, orgName, channelName, policy) {
	...
	try {
		console.log('Performing ' + type + ' transaction on chaincode with ID [' + ccName + '] as organization [' + orgName + '] ...');
		...
		const ccPath = path.join(__dirname, '../chaincode', ccName, ccType);
		const proposalRequest = buildChaincodeProposal(client, ccName, ccPath, version, ccType, args, upgrade, transientMap, policy);
		...
		eventhubs.forEach((eh) => {
			const txPromise = new Promise((resolve, reject) => {
				...
				eh.registerTxEvent(deployId.toString(), (tx, code) => {
					clearTimeout(handle);
					if (code !== 'VALID') {
						console.error('The chaincode ' + type + ' transaction was invalid, code = ' + code);
					...
					clearTimeout(handle);
					console.error('There was a problem with the ' + type + ' transaction event ' + JSON.stringify(err));
		...
		if (results && !(results[0] instanceof Error) && results[0].status === 'SUCCESS') {
			console.log('Successfully performed ' + type + ' transaction on chaincode with ID [' + ccName + ']');
			return await testUtil.sleep(3000);
		} else {
			console.error('Failed to order the ' + type + 'transaction. Error code: ' + results[0].status);
			throw new Error('Failed to order the ' + type + 'transaction. Error code: ' + results[0].status);
		}
	} catch (err) {
		console.error('Failed to perform ' + type + ' instantiation on chaincode with ID [' + ccName + ']');
		throw new Error('Failed to perform ' + type + ' instantiation on chaincode with ID [' + ccName + '] due to error: ' + err.stack ? err.stack : err);
	}
}

function buildChaincodeProposal(client, ccName, ccPath, version, ccType, args, upgrade, transientMap, policy) {
