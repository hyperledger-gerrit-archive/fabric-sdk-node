...
async function installChaincode(ccName, ccType, ccVersion, tls, ccp, orgName, channelName) {
	...
	try {
		...
		testUtil.logMsg('Installing chaincode with ID [' + ccName + '] on ' + orgName + ' peers [' + ccp.getPeersForOrganization(orgName).toString() + ']');
		...
		testUtil.error('Failed to install chaincode');
		throw err;
	}
}
