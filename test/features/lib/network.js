...
async function disconnectGateway(gatewayName) {
	try {
		...
		console.error('disconnectGateway failed with error ', err);
		throw new Error('disconnectGateway failed with error ' + err);
	}
}
...
async function retrieveContractFromGateway(gateway, channelName, chaincodeId) {
	try {
		...
		throw new Error('createContract failed with error ' + err);
	}
}

/**
 * Perform a submit or execute transaction using the network API
 ...
 * @param {Boolean} submit flag to idicate if a submit transaction (true) or execute (false)
 * @return {Object} resolved Promise if a submit transaction; execute result if not
 */
async function performGatewayTransaction(gatewayName, ccName, channelName, args, submit) {
	...
	try {
		if (submit){
			console.log('Submitting transaction [' + func + '] ...');
			await contract.submitTransaction(func, ...funcArgs);
			console.log('Successfully submitted transaction [' + func + ']');
		} else {
			console.log('Executing transaction [' + func + '] ...');
			const result = await contract.executeTransaction(func, ...funcArgs);
			console.log('Successfully executed transaction [' + func  + '] with result [' + result + ']');
...
async function disconnectAllGateways() {
	try {
		...
		console.error('disconnectAllGateways() failed with error ', err);
		throw new Error('disconnectAllGateways() failed with error ' + err);
	}
}
