...
async function create_channels(configPath, ccp, tls) {
	...
	try {
		...
		return Promise.reject(new Error('Fabric: Create channel failed'));
	}
}
...
async function join_channel(ccp, tls, channelName, orgName) {
	...
	try {
		console.log('Joining organization [' + orgName + '] to channel [' + channelName + '] ...');
		...
		if (!(results && results[0] && results[0].response && results[0].response.status === 200)) {
			...
			await testUtil.sleep(3000);
			console.log('Successfully joined organization [' + orgName + '] to channel [' + channelName + ']');
			return client;
		}
	} catch (err) {
		console.log(err);
