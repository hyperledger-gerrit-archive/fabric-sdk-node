...
test('\n\n ** Chain initializeChannel() tests **\n\n', function (t) {
	var c = new Chain('initializeChannel', client);
	var orderer = new Orderer('grpc://localhost:7050');

	var p1 = c.initializeChannel({}
	).then(function () {
	...
	.then(
		...
		function (err) {
			t.fail('Chain initializeChannel() tests, Promise.all: ');
			console.log(err.stack ? err.stack : err);
			t.end();
		}
	);
...
test('\n\n ** Chain sendJoinChannelProposal() tests **\n\n', function (t) {
	var c = new Chain('sendJoinChannelProposal', client);
	var orderer = new Orderer('grpc://localhost:7050');

	var p1 = c.sendJoinChannelProposal({}
	).then(function () {
		t.fail('Should not have been able to resolve the promise because of orderer missing');
	}).catch(function (err) {
		if (err.message.indexOf('Missing orderer') >= 0) {
	...
	var p2 = c.sendJoinChannelProposal(
	).then(function () {
		t.fail('Should not have been able to resolve the promise because of missing request parameter');
	}).catch(function (err) {
		if (err.message.indexOf('Missing all') >= 0) {
	...
	var p3 = c.sendJoinChannelProposal({}
	).then(function () {
	...
	var p4 = c.sendJoinChannelProposal({targets: 'targets'}
	).then(function () {
	...
	var p5 = c.sendJoinChannelProposal({targets: 'targets' , txId : 'txId' }
	).then(function () {
