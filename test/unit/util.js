...
function getMember(username, password, client, t, userOrg) {
	...
	t.comment('getMember, client.getUserContext('+username+', true)');
	...
	.then((user) => {
		return new Promise((resolve, reject) => {
			...
			if (userOrg) cryptoSuite = member.newCryptoSuite({path: module.exports.storePathForOrg(ORGS[userOrg].name)});
...
function getAdmin(client, t, userOrg) {
	...
	var ksOpts = null;
	if (userOrg) ksOpts = {path: module.exports.storePathForOrg(ORGS[userOrg].name)};
