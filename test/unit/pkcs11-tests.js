...
cryptoUtils.generateKey({ algorithm: 'AES', ephemeral: true })
    .then(function(key) {
	console.log(' #1 --- AES generated key: ' + util.inspect(key, { depth: null }));
	var ski = key.getSKI();
	console.log(' #1 --- AES ski[' + ski.length + ']: ' + ski.toString('hex'));
	console.log(' #1 --- AES looked up key: ' + util.inspect(cryptoUtils.getKey(ski),
							 { depth:null }));
	/*
	 * Encrypt a message.
	 */
...
cryptoUtils.getKey(Buffer.from("7acff94187a4bbdf89b670c366bbfe2e0522856e0e86d0890daac3f8e3b96182", 'hex'))
    .then(function(key) {
	console.log(' #2 --- AES looked up key: ' + util.inspect(key, { depth: null }));
	/*
	 * Encrypt a message.
	 */
	var cipherText = cryptoUtils.encrypt(key, Buffer.from('Hello World!!'), {});
...
cryptoUtils.importKey(Buffer.from('7430b92d84e1e3da82c06aff0801aa45f4a429e73f59bfc5141e205617a30387', 'hex'), { algorithm: 'AES' })
    ...
    .then(function(param) {
	/*
	 * Encrypt with software crypto, should get back same bytes (minus 16-byte IV).
	 */
...
cryptoUtils.generateKey({ algorithm: 'ECDSA', ephemeral: true })
    .then(function(key) {
	console.log(' #4 --- ECDSA generated key: ' + util.inspect(key, { depth: null }));
	var ski = key.getSKI();
	console.log(' #4 --- ECDSA ski[' + ski.length + ']: ' + ski.toString('hex'));
	console.log(' #4 --- ECDSA looked up key: ' + util.inspect(cryptoUtils.getKey(ski),
								   { depth:null }));
	/*
	 * Sign a message.
	 */
	var sig = cryptoUtils.sign(key, Buffer.from('Hello World!'), null);
	console.log(' #4 --- ECDSA signature[' + sig.length + ']: ' + sig.toString('hex'));
	return { key, sig };
    })
    .then(function(param) {
	/*
...
cryptoUtils.getKey(Buffer.from('88ee070ac9f2dec7f77ccb64ecf3d58754e05b297ceb7814569f585ef5fa252d', 'hex'))
    .then(function(key) {
	console.log(' #5 --- ECDSA looked up key: '+ util.inspect(key, { depth: null }));
	/*
	 * Sign a message.
	 */
	var sig = cryptoUtils.sign(key, Buffer.from('Hello World!'), null);
	console.log(' #5 --- ECDSA signature[' + sig.length + ']: ' + sig.toString('hex'));
	return { key, sig };
    })
    .then(function(param) {
	/*
