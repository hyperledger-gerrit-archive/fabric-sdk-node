...
const KeyValueStore = require('fabric-client/lib/impl/CryptoKeyStore.js');
...
test('\n\n** CryptoKeyStore tests - couchdb based store tests - use configSetting **\n\n', async (t) => {
	utils.setConfigSetting('key-value-store', 'fabric-ca-client/lib/impl/CouchDBKeyValueStore.js');
	...
	const store = await utils.newCryptoKeyStore({name: dbname, url: 'http://localhost:5985'});
	try {
...
test('\n\n** CryptoKeyStore tests - couchdb based store tests - use constructor argument **\n\n', async (t) => {
	...
	const store = await utils.newCryptoKeyStore(CouchDBKeyValueStore, {name: dbname, url: 'http://localhost:5985'});
	try {
		await testKeyStore(store, t);
		t.end();
	} catch (err) {
...
test('\n\n** CryptoKeyStore tests - newCryptoKeyStore tests **\n\n', async (t) => {
	...
	let cs = await utils.newCryptoKeyStore(config);
	const _testMsg = 'Returned instance should have stored kvsInstance';
	t.equal(cs instanceof KeyValueStore, true, _testMsg);

	cs = await utils.newCryptoKeyStore();
	t.equal(cs instanceof KeyValueStore, true, _testMsg);

	const kvsImplClass = require(utils.getConfigSetting('key-value-store'));
	cs = await utils.newCryptoKeyStore(kvsImplClass);
	t.equal(cs instanceof KeyValueStore, true, _testMsg);

	cs = await utils.newCryptoKeyStore(kvsImplClass, config);
	t.equal(cs instanceof KeyValueStore, true, _testMsg);
