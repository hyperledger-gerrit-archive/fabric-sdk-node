...
const {KeyValueStore} = require('fabric-client/lib/api.js');
...
test('\n\n** CryptoKeyStore tests **\n\n', async (t) => {
	...
	const store = await new CKS({path: keystorePath});
	try {
...
test('\n\n** CryptoKeyStore tests - couchdb based store tests - use configSetting **\n\n', async (t) => {
	...
	const store = await new CKS({name: dbname, url: 'http://localhost:5985'});
	try {
...
test('\n\n** CryptoKeyStore tests - couchdb based store tests - use constructor argument **\n\n', async (t) => {
	...
	const store = await new CKS(CouchDBKeyValueStore, {name: dbname, url: 'http://localhost:5985'});
	try {
		await testKeyStore(store, t);
		t.end();
	} catch (err) {
...
test('\n\n** CryptoKeyStore tests - newCryptoKeyStore tests **\n\n', async (t) => {
	...
	cs = await new CKS(kvsImplClass);
	t.equal(cs instanceof KeyValueStore, true, _testMsg);

	cs = await new CKS(kvsImplClass, config);
	t.equal(cs instanceof KeyValueStore, true, _testMsg);
