...
test('\n\n** CryptoKeyStore tests **\n\n', async (t) => {
	...
	const store = utils.newCryptoKeyStore({path: keystorePath});
	try {
...
test('\n\n** CryptoKeyStore tests - couchdb based store tests - use configSetting **\n\n', async (t) => {
	...
	const store = utils.newCryptoKeyStore({name: dbname, url: 'http://localhost:5985'});
	try {
...
test('\n\n** CryptoKeyStore tests - couchdb based store tests - use constructor argument **\n\n', async (t) => {
	...
	const store = utils.newCryptoKeyStore({name: dbname, url: 'http://localhost:5985'}, CouchDBKeyValueStore);
	try {
		await testKeyStore(store, t);
		t.end();
	} catch (err) {
...
test('\n\n** CryptoKeyStore tests - newCryptoKeyStore tests **\n\n', (t) => {
	...
	t.equal(cs._kvsInstance instanceof KeyValueStore, true, _testMsg);

	cs = utils.newCryptoKeyStore();
	t.equal(cs._kvsInstance instanceof KeyValueStore, true, _testMsg);

	const kvsImplClass = require(utils.getConfigSetting('key-value-store'));
	cs = utils.newCryptoKeyStore(undefined, kvsImplClass);
	t.equal(cs._kvsInstance instanceof KeyValueStore, true, _testMsg);

	cs = utils.newCryptoKeyStore(config, kvsImplClass);
	t.equal(cs._kvsInstance instanceof KeyValueStore, true, _testMsg);
