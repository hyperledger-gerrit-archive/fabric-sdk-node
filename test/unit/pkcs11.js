...
var libpath, pin, slot; // user pin not SO pin
var cryptoUtils;
...
test('\n\n**PKCS11 - locate PKCS11 libpath\n\n', (t) => {
	...
	if (typeof process.env.PKCS11_LIB === 'string' && process.env.PKCS11_LIB !== '' &&
		typeof process.env.PKCS11_PIN === 'string' && process.env.PKCS11_PIN !== '' &&
		typeof process.env.PKCS11_SLOT === 'string' && process.env.PKCS11_SLOT !== '') {

		// use the values passed in from environment variables
		...
		// use conventional defaults for the PIN and slot
		slot = 0;
		pin = '98765432';
	}
