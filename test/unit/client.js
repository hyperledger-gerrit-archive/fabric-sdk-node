...
test('\n\n ** createUser error path - missing required cryptoContent string **\n\n', function (t) {
	var msg = 'Client.createUser parameter \'opts cryptoContent\' string must not be empty.';
	...
	.then((store) => {
		...
		return client.createUser({cryptoContent: '', username: 'anyone', mspid: 'one'});
	}, (err) => {
		logger.error(err.stack ? err.stack : err);
		throw new Error('Failed createUser.');
	}).then((user) => {
...
test('\n\n ** createUser error path - missing required cryptoContent signedCertPEM **\n\n', function (t) {
	var msg = 'Client.createUser parameter \'opts cryptoContent privateKeyStorePEM and signedCertPEM\' strings are required when cryptoContent is not a path string.';
	...
	.then((store) => {
		...
		return client.createUser({cryptoContent: {privateKeyStorePEM: 'abcd'}, username: 'anyone', mspid: 'one'});
	}, (err) => {
		logger.error(err.stack ? err.stack : err);
		throw new Error('Failed createUser.');
	}).then((user) => {
...
test('\n\n ** createUser error path - missing required cryptoContent privateKeyStorePEM **\n\n', function (t) {
	var msg = 'Client.createUser parameter \'opts cryptoContent privateKeyStorePEM and signedCertPEM\' strings are required when cryptoContent is not a path string.';
