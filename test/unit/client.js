...
test('\n\n ** Client.js Tests: user persistence and loading **\n\n', async (t) => {
	...
	try {
		...
		if (e.message === 'Cannot save user to state store when userContext is null.')
			t.pass('Client tests: Should throw "Cannot save user to state store when userContext is null."');
		else t.fail('Client tests: Unexpected error message thrown, should throw "Cannot save user to state store when userContext is null." ' + e.stack ? e.stack : e);
	}
	try {
	...
	if (response && response.getName() === 'someUser')
		t.pass('Client tests: getUserContext not persisted was successful.');
	else
		t.fail('Client tests: getUserContext not persisted was not successful.');
...
test('\n\n ** client installChaincode() tests **\n\n', async (t) => {
	...
	try {
		...
		if (e.message.includes('Missing chaincodePath parameter')) {
			t.pass('Successfully caught missing chaincodePath error');
		} else {
			t.fail(`Failed to catch the missing chaincodePath error. Error: ${e.stack ? e.stack : e}`);
		}
	}
	...
	try {
		...
		if (e.message.includes('Missing "chaincodeVersion" parameter in the proposal request')) {
			t.pass('Successfully caught missing chaincodeVersion error');
		} else {
			t.fail(`Failed to catch the missing chaincodeVersion error. Error: ${e.stack ? e.stack : e}`);
		}
	}
...
test('\n\n ** Test per-call timeout support [client] **\n', (t) => {
	...
	const _getChaincodePackageData = ClientRewired.__set__(
	...
	const p = client.installChaincode({
