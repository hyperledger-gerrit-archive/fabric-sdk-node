...
test('\n\n ** testing get and new peer calls on client **\n\n', function (t) {
	...
	t.doesNotThrow(
			function() {
				var peer = client.getPeer('grpc://somehost:9090');
			},
			null,
			'Should be able to call "getPeer" with a valid URL if already defined');
	t.throws(
			function() {
				var peer = client.newPeer('grpc://somehost:9090');
			},
			/^Error: Peer already exists with this address/,
			'Should not be able to call "newPeer" on the same address');
	t.throws(
			function() {
				var peer = client.getPeer('grpc://somehost:9091');
			},
			/^Error: Peer not found for url/,
			'Should not be able to call "getPeer" on the same address');
	t.end();
});
...
test('\n\n ** testing get and new orderer calls on client **\n\n', function (t) {
	...
	t.doesNotThrow(
			function() {
				var orderer = client.getOrderer('grpc://somehost:9090');
			},
			null,
			'Should be able to call "getOrderer" with a valid URL if already defined');
	t.throws(
			function() {
				var orderer = client.newOrderer('grpc://somehost:9090');
			},
			/^Error: Orderer already exists with this address/,
			'Should not be able to call "newOrderer" on the same address');
	t.throws(
			function() {
				var orderer = client.getOrderer('grpc://somehost:9091');
			},
			/^Error: Orderer not found for url/,
			'Should not be able to call "getPeer" on the same address');
	t.end();
});
