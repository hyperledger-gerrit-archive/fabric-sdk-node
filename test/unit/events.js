...
var CCEventPromise = require('hfc/lib/CCEventPromise.js');
...
test('End-to-end flow of chaincode deploy, transaction invocation, and query', function(t) {
	...
	if (steps.length === 0 || steps.indexOf('step2') >= 0) {
		promise = promise.then(
			...
			function(results) {
				...
				for(var i in proposalResponses) {
					...
					if (proposalResponses && proposalResponses[i].response && proposalResponses[i].response.status === 200) {
						one_good = true;
						logger.info('event send proposal was good');
					} else {
						logger.error('event send proposal was bad');
					}
					all_good = all_good & one_good;
				}
				if (all_good) {
					...
					var ccep = new CCEventPromise();
					return ccep.trigger(chain, chaincode_id, "^evtsender*", 15000, function () { chain.sendTransaction(request); });
			...
			function() {
				console.log("chaincode event recieved");
			},
			function() {
				console.log("chaincode event not recieved");
			}
		);

		if (steps.length >= 1 && steps[steps.length - 1] === 'step2') {
			promise = promise.then(
				function() {
					t.end();
				}
	...
	if (steps.length === 0 || steps.indexOf('step3') >= 0) {
		promise = promise.then(
			...
			function(data) {
				...
				// if coming directly to this step
				// then "data" will be the webUser
				if (typeof data !== 'undefined' && data !== null) {
					webUser = data;
				}
			...
			function(response_payloads) {
				for(let i = 0; i < response_payloads.length; i++) {
        				t.equal(response_payloads[i].toString('utf8'), '1', "Successfully queried correct number of events generated.");
				}
					t.end();
			},
			function(err) {
