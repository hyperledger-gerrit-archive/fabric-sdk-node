...
test('\n\n** ChannelEventHub tests\n\n', (t) => {
	...
	t.throws(
		...
		/Connect failed - no registered event listeners/,
		'Must have registered event listeners before allow connect'
	);
	...
	t.throws(
		...
		/startBlock parameter must be valid integer/,
	...
	t.throws(
		...
		/endBlock parameter must be valid integer/,
		'Check that we able to see end block is not a number'
	);

	var convertToNumber = RewiredChannelEventHub.__get__('convertToNumber');

	let converted = convertToNumber('1');
	if(converted.equals(Long.fromValue(1))) {
		t.pass('Successfully convertToNumber strings to long');
	} else {
		t.fail('convertToNumber did not work for strings')
	}

	converted = convertToNumber(1);
	if(converted.equals(Long.fromValue(1))) {
		t.pass('Successfully convertToNumber integer to long');
	} else {
		t.fail('convertToNumber did not work for integer')
	}

	converted = convertToNumber('aaa');
	if(converted === null) {
		t.pass('Successfully convertToNumber a non number to null');
	} else {
		t.fail('convertToNumber did not work for non number')
	}

	let some_long = Long.fromValue(2)
	converted = convertToNumber(some_long);
	if(converted.equals(Long.fromValue(2))) {
		t.pass('Successfully convertToNumber a long');
	} else {
		t.fail('convertToNumber did not work for long')
	}
