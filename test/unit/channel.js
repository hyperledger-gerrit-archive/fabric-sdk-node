...
test('\n\n*** Test per-call timeout support ***\n', function(t) {
	let stub = sinon.stub(Peer.prototype, 'sendProposal');
	sinon.stub(Policy, 'buildPolicy').returns(Buffer.from('dummyPolicy'));
	...
	sinon.stub(clientUtils, 'buildHeader').returns(Buffer.from('dummyHeader'));
	sinon.stub(clientUtils, 'buildProposal').returns(Buffer.from('dummyProposal'));
	sinon.stub(clientUtils, 'signProposal').returns(Buffer.from('dummyProposal'));
	client._userContext = {
