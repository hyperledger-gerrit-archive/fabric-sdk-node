...
test('\n\n ** Channel joinChannel() tests **\n\n', function (t) {
	...
	var p4 = c.joinChannel({txId : 'txid', block : 'something'}
		...
		if (err.message.indexOf('"target" parameter not specified and no peers are set on Channel') >= 0) {
	...
	var p8 = c.getGenesisBlock({targets: 'targets'}
	).then(function () {
		t.fail('Should not have been able to resolve the promise because of txId request parameter');
	}).catch(function (err) {
		if (err.message.indexOf('Missing txId') >= 0) {
			...
			t.fail('p8 - Failed to catch the missing txId request error. Error: ');
			logger.error(err.stack ? err.stack : err);
		}
	});
