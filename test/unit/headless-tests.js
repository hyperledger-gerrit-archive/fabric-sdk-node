 ...
 */
test('\n\n ** Config **', function(t) {
	console.log('\n NOTE: Run with the following\n       TEST_3=env test_6=mapped node test/unit/headless-tests.js --test-4=argv --test-5=argv\n\n');
	t.equals(hfc.getConfigSetting('test-1', 'notfound'), 'default', 'checking that able to get test_1 value from an additional configuration file');
	//try adding another config file
	hfc.addConfigFile('config/local.json');
	t.equals(hfc.getConfigSetting('test-1', 'notfound'), 'default', 'checking that able to get test-1 value from an additional configuration file');
...
test('\n\n ** Logging utility tests - built-in logger', function(t) {
	...
	var output = '';
	// ===> need to call with --hfc-logging={"debug":: "console"} to see it catch invalid format
	// internal call. clearing the cached logger.
	global.hfc.logger = undefined;
	console.log('\n NOTE: Run with the following\n       TEST_3=env test_6=mapped node test/unit/headless-tests.js --test-4=argv --test-5=argv -hfc-logging={"debug" :: "console"}\n\n');
	try {
		let unhook = intercept(function(txt) {
			output += txt;
		});
