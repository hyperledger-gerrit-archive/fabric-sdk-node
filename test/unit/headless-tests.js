...
test('\n\n ** CryptoSuite_ECDSA_AES - function tests **\n\n', function(t) {
	...
	.then(function(key) {
		t.equal('secp384r1', key.getPublicKey().curveName,
	...
	.then(function(key) {
		t.equal('secp256r1', key.getPublicKey().curveName,
	...
	.then(function(key) {
		...
		var testSignature = function(msg) {
			...
			if (sig) {
				...
				var pubKey = cryptoUtils._ecdsa.keyFromPublic(key.getPublicKey().pubKeyHex, 'hex');
				// note that the signature is generated on the hash of the message, not the message itself
				t.equal(pubKey.verify(cryptoUtils.hash(msg), sig), true,
...
test('\n\n ** ECDSA Key Impl tests **\n\n', function(t) {
	...
	t.equal(key1.toBytes().length, 246, 'Checking toBytes() output');
