...
test('\n\n ** configuration testing **\n\n', function (t) {
	...
	t.doesNotThrow(
		() => {
			...
			peers = client._getPeersForOrgOnChannel(channel);
			...
			peers = client._getPeersForOrgOnChannel();
			t.equals(1, peers.length,  ' Check to see that we got 1 peer');
			t.equals('grpcs://localhost:7051', peers[0].getUrl(),  ' Check to see if we got the right peer for org1 that is endorsing and on the channel');
			...
			t.equals(client2._getPeersForOrgOnChannel(channel3).length, 2, ' Check to see that we got 2 peers for Org2 on "mychannel3"');
			...
			t.equals(client2._getPeersForOrgOnChannel(channel3).length, 1, ' Check to see that we got 1 peers for Org2 on "mychannel3"');
