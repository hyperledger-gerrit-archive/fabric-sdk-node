...
test('\n\n ** configuration testing **\n\n', function (t) {
	...
	t.doesNotThrow(
		() => {
			...
			t.equals(opts['request-timeout'], 32000, 'check that endorser timeout was added');
			opts = {};
			client._network_config.addTimeout(opts,2);
			t.equals(opts['request-timeout'], 20000, 'check that orderer timeout was added');
			opts = {};
			...
			t.equals(peer._options['request-timeout'],32000, ' check that we get this peer endorser timeout set');
			let orderer = client._network_config.getOrderer('orderer.example.com');
			t.equals(orderer._options['request-timeout'],20000, ' check that we get this orderer timeout set');
