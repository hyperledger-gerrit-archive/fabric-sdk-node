...
describe('Peer', () => {
	...
	describe('#sendDiscovery', () => {
		it('should log on entry', async () => {
			...
			await peer.sendDiscovery().should.be.rejected;
			sinon.assert.calledWith(peerLogger.debug, '%s - Start', 'sendDiscovery');
		});
		...
		it('should log and reject on timeout', async () => {
			...
			sinon.assert.calledWith(peerLogger.error, '%s - timed out after:%s');
		});
		...
		it('should log and reject Error object on discover Response error string', async () => {
			...
			sinon.assert.calledWith(peerLogger.debug, '%s - Received discovery response from: %s status: %s');
		});
		...
		it('should log and reject Error object on null response from discovery', async () => {
			...
			sinon.assert.calledWith(peerLogger.error, '%s - rejecting with:%s');
		});
		...
		it('should log and resolve on good response from discover', async () => {
			...
			sinon.assert.calledWith(peerLogger.debug, '%s - Received discovery response from peer "%s"');
		});
