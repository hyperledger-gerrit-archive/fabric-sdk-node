...
declare namespace Client {
  ...
  export class Channel {
    ...
    close(): void;
    initialize(request?: Buffer | InitializeRequest): Promise<void>;

    getName(): string;
    getDiscoveryResults(): DiscoveryResults;
    refresh(request: DiscoveryRequest): DiscoveryResults;
  ...
  export type DiscoveryEndpoints = { endpoints: DiscoveryEndpoint[] };
  ...
  export interface DiscoveryPeer {
    ...
    name: string; // Added by Channel.initialize
  }
  export type DiscoveryPeers = { peers: DiscoveryPeer[] };

  export type DiscoveryMembershipResults = {
    [name: string]: DiscoveryPeers;
  };

  export type DiscoveryChaincodeResults = {
    [chaincode: string]: DiscoveryChaincodeResult;
  };
  export type DiscoveryChaincodeResult = {
    groups: {
      [group_name: string] : DiscoveryPeers;
    },
    layouts: {
  ...
  export interface DiscoveryResults {
    ...
    orderers?: { [name: string]: DiscoveryEndpoints };

    local_peers?: DiscoveryMembershipResults;
    peers_by_org?: DiscoveryMembershipResults;

    endorsement_targets?: DiscoveryChaincodeResults;

    timestamp: number; // added by Channel.initialize
  }
}
