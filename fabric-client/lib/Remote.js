...
var Endpoint = class {
	constructor(url /*string*/ , pem /*string*/ ) {
		...
		var cert = fs.readFileSync('/Users/jimzhang/workspace/fabric-sdk-node/test/fixtures/config/tls/cert.pem');
		var key = fs.readFileSync('/Users/jimzhang/workspace/fabric-sdk-node/test/fixtures/config/tls/key.pem');
		...
		if (protocol === 'grpc') {
			...
			this.addr = purl.host;
			this.creds = grpc.credentials.createSsl(new Buffer(pem), Buffer.from(key), Buffer.from(cert));
		} else {
			var error = new Error();
