...
class Remote {
	...
	constructor(url, opts = {}) {
		const {
			pem, clientKey, clientCert, name,
			['ssl-target-name-override']: ssl_target_name_override
		} = opts;
		this.clientCert = clientCert;

		// connection options
		this._options = opts;
		delete this._options.pem;
		delete this._options['ssl-target-name-override'];
		...
		} else if (utils.getConfigSetting(MAX_SEND_V10)) {
			grpc_send_max = utils.getConfigSetting(MAX_SEND_V10);
		} else if (utils.getConfigSetting(MAX_SEND)) {
		...
		if (utils.checkIntegerConfig(opts, 'request-timeout')) {
			...
			this._request_timeout = utils.getConfigSetting('request-timeout', this._request_timeout);
		}
