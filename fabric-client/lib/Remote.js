...
var Endpoint = class {
	constructor(url /*string*/ , pem /*string*/ , clientKey /*string*/ , clientCert /*string*/) {
		...
		if (protocol === 'grpc') {
			...
			let pembuf = Buffer.concat([Buffer.from(pem), Buffer.from('\0')]);
			if (clientKey || clientCert){
				// must have both clientKey and clientCert if either is defined
				if (clientKey && clientCert){
					if ((typeof clientKey === 'string') && (typeof clientCert === 'string')) {
						let clientKeyBuf = Buffer.from(clientKey);
						let clientCertBuf = Buffer.concat([Buffer.from(clientCert), Buffer.from('\0')]);
