...
class LifecyclePackager extends BasePackager {
	...
	async finalPackage (chaincodeName, chaincodeVersion, chaincodeType, packageBytes, chaincodePath) {
		...
		logger.debug('%s - Start building final lifecycle package for chaincode:%s version:%s path:%s type:%s',
			method, chaincodeName, chaincodeVersion, chaincodePath, chaincodeType);
		...
		const descriptors = this.buildMetaDataDescriptors(chaincodePath, chaincodeType);
		descriptors.concat(this.buildPackageDescriptors(packageBytes));
	 ...
	 * @param {string} path
	 */
	buildMetaDataDescriptors(type, path) {
		if (!path) {
			path = '';
		}
		const metadata = {
