...
var ChannelEventHub = class {
	...
	constructor(channel, peer) {
		...
		// this will hold the block number to be used to be used when this
		// event hub is connection to the remote peer's channel event sevice
	...
	/*
	 * Internal method
	 * checks that only one registration for replay
	 ...
	 _checkStartBlock(start_block) {
		...
		if(typeof start_block !== 'undefined' && start_block != null) {
			if(Number.isInteger(start_block)) {
				...
				throw new Error('start_block parameter must be valid interger');
			}
		}
		if(have_start_block && this._haveRegistrations()) {
			logger.error('This eventhub is already registered to replay events start_block:',start_block);
	...
	_checkConnection(throw_error, force_reconnect) {
		...
		if(this._connected || this._connect_running) {
			logger.debug('_checkConnection - this hub %s is connected or trying to connect with stream channel state %s', this._peer.getUrl(), state);
		}
		else {
	 ...
	 * @param {Long} start_block - optonal - The starting block number for event checking.
	 *                             When included the event service will be ask to start sending
	 *                             blocks from this block. This would be how to replay missed
	 *                             blocks that were added to the ledger. Since replaying events
	 *                             may confuse other event listeners, only one listener will be
	 *                             allowed on a ChannelEventHub when replay is being used.
