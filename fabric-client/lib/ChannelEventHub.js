	 ...
	 */
	 _checkReplay(options) {
		...
		if(options && typeof options.startBlock !== 'undefined') {
			let number = utils.convertToLong(options.startBlock);
			if(number) {
		...
		if(options && typeof options.endBlock !== 'undefined') {
			let number = utils.convertToLong(options.endBlock);
			if(number) {
				have_end_block = true;
		...
		if((have_start_block || have_end_block) && (this._connected || this._connect_running)) {
			logger.error('This ChannelEventHub has already been connected to start receiving blocks. Not able to resume events with startBlock:%s endBlock:%s',options.startBlock, options.endBlock);
			throw new Error('Event listeners that will replay or resume must be registered before connecting to the peer channel-based event service');
		}
