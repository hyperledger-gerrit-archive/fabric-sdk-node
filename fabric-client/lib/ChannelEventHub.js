...
var ChannelEventHub = class {
			...
			if (deliverResponse.Type === 'block' || deliverResponse.Type === 'filtered_block') {
				...
				try {
					...
					if(deliverResponse.Type === 'block') {
						...
						block = BlockDecoder.decodeFilteredBlock(deliverResponse.filtered_block);
				...
				// got an unknown... means we need to tell
				// all registered users that something is wrong
				self._disconnect(new Error('Received unknown message on the event stream. message:%s',deliverResponse));
			}
		});
	...
	_callChaincodeListener(chaincode_event, block_num, tx_id, val_code) {
