 ...
 * <br><br><br>
 * The fabric-client may miss the publishing of blocks if the fabric-client crashes.
 * It will be helpful to record the block numbers received, such that
 * when the next block arrives and its number is not the next in sequence, then
 * the application knows exactly which blocks have been missed. It can then use
 * [queryBlock]{@link Channel#queryBlock} to get those missed blocks from the
 * target peer. <br> or <br>
 * Register a listener using the startBlock option to resume or replay the blocks.
 * You may also include an endBlock number if you wish to stop listening. Using the
 * replay allows you to use the same code that was written to handle a live
 * event.
 *
 * @example
...
class ChannelEventHub {
	...
	/**
	 * Is this instance connected to the fabric service?
	 ...
	 *        to the peer. This option is useful when the signing of requests to
	 *        the fabric network is done within the browser application and not
	 *        within a Node.js server application.
	 */

	/**
	 * Establishes a connection with the peer fabric service.
	 *
	...
	_connect(request) {
		...
		const connecton_setup_timeout = setTimeout(() => {
			...
			self._disconnect(new Error('Unable to connect to the peer fabric service'));
		...
		this._stream.on('end', () => {
			...
			self._disconnect(new Error('Peer fabric service has disconnected due to an "end" event'));
		});
	...
	/**
	 * Disconnects the ChannelEventHub from the peer fabric service.
	 * Will close all event listeners and send an Error object
	 * with the message "ChannelEventHub has been shutdown" to
	 * all listeners that provided an "onError" callback.
	 */
	...
	/*
	 * Internal method
	 * Disconnects the connection to the peer fabric service.
	 ...
	 *           different options. The a registration with a startBlock must be
