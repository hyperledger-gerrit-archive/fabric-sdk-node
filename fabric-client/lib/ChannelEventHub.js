 ...
 *     register for events using the startBlock option to resume / replay the events. You may
...
var ChannelEventHub = class {
	...
	constructor(channel, peer) {
		...
		this._ending_block_number = null; //default is max if using
		...
		this._stream = null;
		//open this event hub to registrations
	 ...
	 * If this event hub has been configured for a resume/replay of events then
	...
	connect(){
		...
		// do not allow connect if no registered listeners
		if(!this._haveRegistrations()) {
			throw new Error('Connect failed - no registered event listeners');
		}
		this._connect_running = false; //override a running connect
		this._allowRegistration = false; //close this event hub to registrations
		...
		this._stream.on('data', function(event) {
			...
			if (event.Type === 'block') {
				...
				if(self._ending_block_seen) {
					...
					self._disconnect(new Error(util.format('Received status message on a event stream that should have stayed open status:%s',event.status)));
				}
			}
			else {
	...
	/*
	 * Internal method
	 * checks the replay resume options
	 ...
	 */
	 _checkReplay(options) {
		...
		if(options && typeof options.startBlock !== 'undefined') {
			let number = convertToNumber(options.startBlock);
			if(number) {
				have_start_block = true;
				converted_options.start_block = number;
			} else {
				throw new Error('startBlock parameter must be valid integer ::' + options.startBlock);
			}
		}
		if(options && typeof options.endBlock !== 'undefined') {
			let number = convertToNumber(options.endBlock);
			if(number) {
				have_end_block = true;
				converted_options.end_block = number;
			} else {
				throw new Error('endBlock parameter must be valid integer ::' + options.endBlock);
			}
		}

		if((have_start_block || have_end_block) && this._haveRegistrations()) {
			logger.error('This ChannelEventHub is already registered with active listeners. Not able to resume events with startBlock:%s endBlock:%s',options.startBlock, options.endBlock);
			throw new Error('Only one event registration is allowed when replaying events');
		}

		if(have_end_block) {
			if(have_start_block && converted_options.start_block > converted_options.end_block) {
			...
			logger.debug('_checkReplay - Event replay will end at block %s', converted_options.end_block);
		}
		if(have_start_block) {
			this._starting_block_number = converted_options.start_block;
			this._allowRegistration = false;
			logger.debug('_checkReplay - Event replay will start at block %s', converted_options.start_block);
		}
	}
	...
	checkConnection(force_reconnect) {
		logger.debug('_checkConnection - start force_reconnect:%s', force_reconnect);
			...
			try {
				if(this._stream) {
					...
					if(is_paused) {
						...
						// try to reconnect
						this._connect_running = false; //force a restart
						this._connect(true);
					}
				}
				else {
	 ...
	 */
	  ...
	  *           This is how to resume and replay missed blocks that were added
	  *           to the ledger.
	  *           Default is the latest block on the ledger.
	  *           Replaying events may confuse other event listeners,
	  *           therefor only one listener will be allowed on a ChannelEventHub
	  *           when resume and replay is being used.
	  ...
	  *           Default is MAX integer, which is the block number of the last
	  *           block that will be committed to the ledger.
	  *           Replaying events may confuse other event listeners,
	  *           therefor only one listener will be allowed on a ChannelEventHub
	  *           when resume and replay is being used.
	  ...
	  *           The default for this setting will be same as the unregister setting.
	  */
	...
	registerChaincodeEvent(ccid, eventname, onEvent, onError, options) {
		...
		this._checkReplay(options);

		let chaincode_reg = new ChaincodeRegistration(ccid, eventname, onEvent, onError, options);
	...
	registerBlockEvent(onEvent, onError, options) {
		...
		let block_registration = new EventRegistration(onEvent, onError, options, true, true);
	...
	registerTxEvent(txid, onEvent, onError, options) {
		...
		let trans_reg = new EventRegistration(onEvent, onError, options, true, false);
	 ...
	 * in the range when this event hub is in replay resume mode
	 */
...
function convertToNumber(value) {
	...
	try {
		if(typeof value !== 'undefined' && value != null) {
			...
			// Long will return a zero for invalid strings
			if(result.equals(Long.ZERO)) {
				if(Number.isInteger(value) || value === '0') {
					logger.debug('convertToNumber - found a zero');
				} else {
		...
		logger.debug('convertToNumber - value is not valid - %s',error);
	}
	return result;
}
...
var ChaincodeRegistration = class {
	 ...
	 */
	constructor(ccid, eventNameFilter, onEvent, onError, options) {
		// chaincode id
		this.ccid = ccid;
		// event name regex filter
		this.eventNameFilter = new RegExp(eventNameFilter);

		this.event_reg = new EventRegistration(onEvent, onError, options, false, false);
	}
};
