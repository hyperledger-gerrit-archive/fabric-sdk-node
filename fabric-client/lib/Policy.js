...
const MSPPrincipal = grpc.load(__dirname + '/protos/msp/msp_principal.proto').common.MSPPrincipal;
const MSPRole = grpc.load(__dirname + '/protos/msp/msp_principal.proto').common.MSPRole;

const SignaturePolicyEnvelope = grpc.load(__dirname + '/protos/common/policies.proto').common.SignaturePolicyEnvelope;
const SignaturePolicy = grpc.load(__dirname + '/protos/common/policies.proto').common.SignaturePolicy;
		...
		if (!policy) {
			...
			for (const name in msps) {
				if (msps.hasOwnProperty(name)) {
					const onePrn = new MSPPrincipal();
					onePrn.setPrincipalClassification(MSPPrincipal.Classification.ROLE);

					const memberRole = new MSPRole();
					memberRole.setRole(MSPRole.MSPRoleType.MEMBER);
			...
			const oneOfAny = new SignaturePolicy.NOutOf();
			oneOfAny.setN(1);
			oneOfAny.setRules(signedBys);

			const noutof = new SignaturePolicy();
...
function buildPrincipal(identity) {
	...
	if (principalType === IDENTITY_TYPE.Role) {
		...
		if (roleName === 'peer') {
			newRole.setRole(MSPRole.MSPRoleType.PEER);
		} else if (roleName === 'member') {
			newRole.setRole(MSPRole.MSPRoleType.MEMBER);
		} else if (roleName === 'admin') {
			newRole.setRole(MSPRole.MSPRoleType.ADMIN);
		} else {
