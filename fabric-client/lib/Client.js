...
var Client = class {
	 ...
	 * @throws {Error} if the peer by that address already exists in the application's local cache
	 */
	newPeer(url, opts) {
		var peer = this._peers[url];

		if (peer)
			throw new Error(util.format('Peer already exists with this address : %s', url));

		peer = new Peer(url, opts);
		this._peers[url] = peer;
		return peer;
	}

	/**
	 * Get a {@link Peer} instance from the local cache.
	 * @param {url} url The address of the peer.
	 * @returns {Peer} The peer instance
	 * @throws {Error} if the local cache does not have a peer object with this address.
	 */
	getPeer(url) {
		var ret = this._peers[url];

		if (ret)
			return ret;
		else {
			logger.error('Peer not found for url '+url+'.');
			throw new Error('Peer not found for url '+url+'.');
		}
	}
	 ...
	 * @throws {Error} if the orderer by that address already exists in the application's local cache
	 */
	newOrderer(url, opts) {
		var orderer = this._orderers[url];

		if (orderer)
			throw new Error(util.format('Orderer already exists with this address :%s', url));

		orderer = new Orderer(url, opts);
		this._orderers[url] = orderer;
		return orderer;
	}

	/**
	 * Get a {@link Orderer} instance from the local cache.
	 * @param {url} url The address of the peer.
	 * @returns {Orderer} The orderer instance
	 * @throws {Error} if the local cache does not have an orderer object with this address.
	 */
	getOrderer(url) {
		var ret = this._orderers[url];

		if (ret)
			return ret;
		else {
			logger.error('Orderer not found for url '+url+'.');
			throw new Error('Orderer not found for url '+url+'.');
		}
	}
