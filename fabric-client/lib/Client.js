...
var Client = class extends BaseClient {
	...
	constructor() {
		super();
		this._mspid = null; // The orgainization name
	 ...
	 * create an istance of a peer if there is a loaded connection profile that
	 ...
	 * create an istance of an orderer if there is a loaded connection profile that
			...
			try {
				...
				if(!peers && request.channelNames) {
					...
					for(let i in channel_names) {
						...
						const channel_peers = this._getPeersForOrgOnChannel(channel);
	...
	/*
	 * Internal utility method to get target peers from the network configuration
	 ...
	 * will be returned.
	 */
	_getPeersForOrgOnChannel(channel, org_name) {
		let method = '_getPeersForOrgOnChannel';
		logger.debug('%s - starting',method);
		let peers = [];
		if(this._network_config && this._network_config.hasClient()) {
			const org_peers = this.getPeersForOrg(org_name);
			logger.debug('%s - have client config and an org_peers list with %s',method,org_peers.length);
			if(channel) {
				logger.debug('%s - have channel %s', method, channel);
				const channel_peers = channel.getPeers();
				for(let j in channel_peers) {
					const channel_peer = channel_peers[j];
					for(let k in org_peers) {
						...
						if(org_peer.getName() === channel_peer.getName()) {
							...
							logger.debug('%s - adding peer to list:%s', method, channel_peer.getName());
						}
					}
				}
			} else {
				logger.debug('%s - no channel - return org list',method);
				peers = org_peers;
			}
		}
