 ...
 * with a network of peers and orderers. An application using the SDK may need to interact with
 ...
 *     You can enroll each authenticated user separately so that each user gets its own signing identity. Or multiple
 *     users can share the same signing identity.
...
var Client = class extends BaseClient {
	 ...
	 * configuration bytes passed in, and returns the signatures that is ready to be
	 ...
	 * have each organization's peer nodes to join the channel, by sending the channel
	 ...
	 * @property {Object} index:2 - The original Header object
	 */
	 ...
	 * <br><br>
	 * Chaincode installation is simply uploading the chaincode source and their
	 ...
	 * signing identity (the private key and its corresponding certificate), will be used to sign transaction
	 * and query requests with the blockchain network.
	 * <br><br>
