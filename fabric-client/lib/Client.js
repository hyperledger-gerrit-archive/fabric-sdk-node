...
//var KeyStore = require('./impl/CryptoKeyStore.js');ref via CryptoSuite
//var ecdsaKey = require('./impl/ecdsa/key.js');ref via CryptoSuite
var util = require('util');
var path = require('path');
var fs = require('fs-extra');

//var jsrsa = require('jsrsasign');ref via CryptoSuite
//var KEYUTIL = jsrsa.KEYUTIL;ref via CryptoSuite
...
var Client = class {
	...
	*/
	 ...
	 * 'ca-import' configuration settings.
	 * <br>- username {string} user name used for enrollment
	 * <br>- userOrg {string} organization associated with an MSP
	 * <br>
	 * <br>- mspConfigDir - required - The root directory
	 *       which will be concatenated to the 'ca-import' config settings
	 *       privateKeyStore and signedCertificate.
	 * <br>
	 * <br>See the default 'ca-import' configuration settings in
	 * <br>/fabric-sdk-node/fabric-client/config/default.json.
	 */
	getSubmitter(opts) {
		// Get the ca-import config settings
		var caImport = sdkUtils.getConfigSetting('ca-import', 'notfound');
		logger.info('opts = %s', JSON.stringify(opts));
		logger.debug('caImport = %s', JSON.stringify(caImport));

		if (!opts) {
			return Promise.reject(new Error('Client.getSubmitter missing required \'opts\' parameter.'));
		}
		if (caImport === 'notfound') {
			return Promise.reject(new Error('Client.getSubmitter missing \'ca-import\' configuration settings.'));
		}
		// Assuming that caImport has all of the default settings
		var userOrg = opts && opts.userOrg ? opts.userOrg : caImport.userOrg;
		if (!userOrg) {
			return Promise.reject(new Error('Client.getSubmitter missing parameter, either \'opts userOrg\' or \'ca-import userOrg\' is required.'));
		}
		if (opts && (!opts.username || opts.username && opts.username.length < 1)
			&& (!caImport.orgs[userOrg].username || caImport.orgs[userOrg].username.length < 1)) {
			return Promise.reject(new Error('Client.getSubmitter missing parameter, either \'opts username\' or \'ca-import username\' is required.'));
		}
		var username = opts && opts.username ? opts.username : caImport.orgs[userOrg].username;
		var mspid = caImport.orgs[userOrg].mspid;
		var tlsOptions = caImport.orgs[userOrg].tlsOptions;
		var keyStoreOpts = sdkUtils.getConfigSetting('keyStoreOpts', 'notfound');
		if (keyStoreOpts == 'notfound') {
			keyStoreOpts = { path: caImport.orgs[userOrg].storePath+'-'+caImport.orgs[userOrg].name };
		}
		var KVSImplClass, cryptoSuite;
		var cryptoOpts = { kvsOpts: keyStoreOpts };
		logger.debug('keyStoreOpts: '+JSON.stringify(keyStoreOpts));
		if (!opts || !opts.mspConfigDir) {
			return Promise.reject(new Error('Client.getSubmitter parameter \'opts mspConfigDir\' is required.'));
		}
		var mspConfigDir = opts.mspConfigDir;
		var self = this;
		...
		return new Promise((resolve, reject) => {
			...
			.then((store) => {
				KVSImplClass = store;
				logger.info('then store KVSImplClass: %s',store);
				self.setStateStore(store);
				cryptoSuite = sdkUtils.newCryptoSuite(KVSImplClass/*function*/, keyStoreOpts/*object*/);
				return '';
			}).then(() => {
				...
				var privKeyPEM = path.join(mspConfigDir, caImport.orgs[userOrg].privateKeyStore);
				var pemData, member, importedKey;
				...
				.then((data) => {
					...
					return cryptoSuite.importKey(pemData.toString());
				}).then((key) => {
					logger.debug('then key');
					importedKey = key;
					// next save the certificate in a serialized user enrollment in the state store
					var certPEM = path.join(mspConfigDir, caImport.orgs[userOrg].signedCertificate);
					return readFile(certPEM);
				}).then((data) => {
					logger.debug('then certPEM data');
					member = new User(username);
					return member.setEnrollment(importedKey, data.toString(), mspid, cryptoOpts);
				}).then(() => {
