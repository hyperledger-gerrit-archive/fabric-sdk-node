...
var Client = class extends BaseClient {
	...
	createUser(opts) {
		...
		return new Promise((resolve, reject) => {
			...
			if (!opts) {
				reject(new Error('Client.createUser missing required \'opts\' parameter.'));
			}
			const { username,mspid } = opts;
			if (!username || username.length < 1) {
				reject(new Error('Client.createUser parameter \'opts username\' is required.'));
			}
			if (!mspid || mspid.length < 1) {
				reject(new Error('Client.createUser parameter \'opts mspid\' is required.'));
			}
			if (opts.cryptoContent) {
				if ((opts.cryptoContent.privateKey || opts.cryptoContent.signedCert) &&
					...
					reject(new Error('Client.createUser both parameters \'opts cryptoContent privateKey and signedCert\' files are required.'));
				}
				if ((opts.cryptoContent.privateKeyPEM || opts.cryptoContent.signedCertPEM) &&
					...
					reject(new Error('Client.createUser both parameters \'opts cryptoContent privateKeyPEM and signedCertPEM\' strings are required.'));
				}
			} else {
				reject(new Error('Client.createUser parameter \'opts cryptoContent\' is required.'));
			}
