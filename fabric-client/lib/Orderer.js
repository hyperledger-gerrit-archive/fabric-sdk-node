...
class Orderer extends Remote {
	...
	async sendBroadcast(envelope, timeout) {
		...
		if (!envelope || envelope === '') {
			...
			throw new Error('Missing data - Nothing to broadcast');
		}
		...
		const broadcast = this._ordererClient.broadcast();
	...
	async sendDeliver(envelope) {
		...
		if (!envelope) {
			...
			throw new Error('Missing data - Nothing to deliver');
		}

		await this.waitForReady(this._ordererClient);
		// Send the seek info to the orderer via grpc
		const deliver = this._ordererClient.deliver();
		...
		return new Promise((resolve, reject) => {
			const deliver_timeout = setTimeout(() => {
				logger.debug('sendDeliver - timed out after:%s', this._request_timeout);
				deliver.end();
				return reject(new Error(error_msg));
