...
const Channel = class {
	...
	async initialize(request) {
		...
		if (use_discovery) {
			...
			let discover_results = await this._discover(discover_request);
			...
			return this.loadConfigEnvelope(config_envelope);
		}
	}
	 ...
	 * @returns {Promise<DiscoveryResponse>} The results of the discover
	 */
		...
		if (response && response.results) {
			...
			for (const index in response.results) {
				...
				if (!result) {
					...
					logger.error(`Channel:${this.getName()} received discovery error:${result.error.content}`);
					error_msg = result.error.content;
					break;
				} else {
					logger.debug('%s - process results', method);
					if (result.config_result) {
						const config = this._processDiscoveryConfigResults(result.config_result);
						results.msps = config.msps;
						results.orderers = config.orderers;
					}
					if (result.members) {
						if (request.local && index === 0) {
							results.local_peers = this._processDiscoveryMembershipResults(result.members);
						} else {
							results.peers_by_org = this._processDiscoveryMembershipResults(result.members);
						}
					}
					if (result.cc_query_res) {
						results.endorsement_targets = this._processDiscoveryChaincodeResults(result.cc_query_res);
					}
			...
			if (error_msg) {
				throw Error(`Channel: ${this.getName()} Discovery error:${error_msg}`);
			} else {
				return results;
			}
		} else {
	...
	async getChannelConfig(target, timeout) {
		...
		if (responses && Array.isArray(responses)) {
			...
			if (response instanceof Error) {
				...
				return _configtxProto.ConfigEnvelope.decode(payload.data);
			}
			else {
	...
	async getChannelConfigFromOrderer() {
		...
		const latestBlock = await orderer.sendDeliver(envelope);
		...
		if (latestBlock) {
			...
			throw new Error(`${method} - Failed to retrieve latest block`);
		}

		logger.debug('%s - latest block is block number %s', latestBlock.header.number);
		// get the last config block number
		const metadata = _commonProto.Metadata.decode(latestBlock.metadata.metadata[_commonProto.BlockMetadataIndex.LAST_CONFIG]);
		...
		// this will return us a block
		const configBlock = await  orderer.sendDeliver(envelope);
		if (!configBlock) {
			throw new Error('Config block was not found');
		}
		// lets have a look at the block
		logger.debug('%s -  config block number ::%s  -- numberof tx :: %s', method, configBlock.header.number, configBlock.data.data.length);
		if (configBlock.data.data.length !== 1) {
			throw new Error('Config block must only contain one transaction');
		}
		envelope = _commonProto.Envelope.decode(configBlock.data.data[0]);
		const payload = _commonProto.Payload.decode(envelope.payload);
	 ...
	 *                                       hashes that guarantees the immutability of the ledger
	 ...
	 *
	 * @param {byte[]} blockHash block hash of the Block in question.
	 * @param {Peer} target - Optional. The peer to send the query to. If no target is passed,
	 *                        the query is sent to the first peer that was added to the channel object.
	 * @param {boolean} useAdmin - Optional. Indicates that the admin credentials should be used in making
	 *                  this call to the peer.
	...
	try {
		...
		case 'AnchorPeers': {
			...
			if (anchor_peers && anchor_peers.anchor_peers) {
				for (const peer of anchor_peers.anchor_peers) {
					const anchor_peer = {
						host: peer.host,
						port: peer.port,
						org
