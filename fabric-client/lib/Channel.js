...
var Channel = class {
	...
	joinChannel(request) {
		...
		var self = this;
		let signing_identity = self._client_context.getAdminSigningIdentity();
		...
		var channelHeader = clientUtils.buildChannelHeader(
			...
			'',
			request.txId.getAdminTransactionID(),
			null, //no epoch
			Constants.CSCC
		);

		var header = clientUtils.buildHeader(signing_identity, channelHeader, request.txId.getNonce());
		var proposal = clientUtils.buildProposal(chaincodeSpec, header);
		var signed_proposal = clientUtils.signProposal(signing_identity, proposal);
 ...
 */
	...
	_sendChaincodeProposal(request, command) {
		...
		var header, proposal;
		var signing_identity = self._client_context.getAdminSigningIdentity();
		var user_context =  self._client_context._userContext;
		let lcccSpec_args = [
			Buffer.from(command),
		...
		//header = clientUtils.buildHeader(signing_identity, channelHeader, request.txId.getNonce());
		header = clientUtils.buildHeader(user_context.getIdentity(), channelHeader, request.txId.getNonce());
		proposal = clientUtils.buildProposal(lcccSpec, header, request.transientMap);
		//let signed_proposal = clientUtils.signProposal(signing_identity, proposal);
		let signed_proposal = clientUtils.signProposal(user_context.getSigningIdentity(), proposal);
	...
	sendTransaction(request) {
		...
		let proposal_header = _commonProto.Header.decode(chaincodeProposal.getHeader());
		...
		let signature_header = proposal_header.getSignatureHeader();
		transactionAction.setHeader(signature_header);
		...
		var userContext = this._client_context.getUserContext();
		var signing_identity = this._client_context.getAdminSigningIdentity();

		let signature_header_proto = _commonProto.SignatureHeader.decode(signature_header);
		var payload = new _commonProto.Payload();
		//let header = clientUtils.buildHeader(userContext.getIdentity(), proposal_header.getChannelHeader(), signature_header_proto.getNonce().toBuffer());
		//let header = clientUtils.buildHeader(signing_identity, proposal_header.getChannelHeader(), signature_header_proto.getNonce());
		payload.setHeader(proposal_header);
		payload.setData(transaction.toBuffer());

		let payload_bytes = payload.toBuffer();
		var sig = null;
		if(request.admin) {
			sig = signing_identity.sign(payload_bytes);
		} else {
			sig = userContext.getSigningIdentity().sign(payload_bytes);
		}
		let signature = Buffer.from(sig);
