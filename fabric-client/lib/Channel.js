...
var Channel = class {
	 ...
	 *
	 * @param {TransactionRequest} request
	 * @returns An array of Promise instances, the last one of which comes from "BroadcastResponse" message returned by the orderer that contains a single "status" field for a starndard [HTTP response code]{@link https://github.com/hyperledger/fabric/blob/v1.0.0/protos/common/common.proto#L27}.
			...
			.then(
				function (results) {
					...
					for (let i in proposalResponses) {
						...
						if (!that.verifyProposalResponse(proposalResponses[i])) {
					...
					if (allGood) {
						let ok = that.compareProposalResponseResults(proposalResponses);
						if (ok) {
					...
					if (allGood) {
						...
						let sendPromise = that.sendTransaction(ordererRequest);
