...
const Channel = class {
	...
	async sendTokenCommand(request, timeout) {
		...
		if (!request.txId) {
			throw new Error(util.format('Missing "txId" in request on the %s call', method));
		}
		if (!request.tokenCommand) {
			throw new Error(util.format('Missing "tokenCommand" in request on the %s call', method));
		}
		...
		logger.info('sendTokenCommand, number targetPeers=%d', targetPeers.length);
		...
		if (this._prover_handler) {
			...
			const response = await this._token_handler.processCommand(params);
			return response;
		} else {
	...
	static _buildSignedTokenCommand(request, channelId, client_context) {
		...
		request.tokenCommand.setHeader(header);
		const signed_command = token_utils.signCommand(signer, request.tokenCommand);
	 ...
	 *           when not provided, the transaction will be sent to the orderers in the channel.
	 */
