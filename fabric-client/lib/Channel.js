...
const Channel = class {
	 ...
	 *           and optional for [generateEndorseProposal]{@link Channel#generateEndorseProposal}
	 * @property {map} transientMap - Optional. <string, byte[]> map that can be
	...
	/**
	 * @typedef {Object} EndorseProposal
	 ...
	 * @property {Buffer} argbytes - Optional. sometimes there might be argbytes.
	 * @property {map} transientMap - Optional. <sting, byte[]> map map that can be
	 ...
	 * the signed proposal to peer by [sendSignedEndorseProposal]
	 *
	 * so the user's private
	 * key would not be required at SDK side.
	 *
	 * @param {EndorseProposal} request chaincode invoke request
	 ...
	 */
	generateEndorseProposal(request, mspId, certificate, admin) {
		const method = 'generateEndorseProposal';
		logger.debug('%s - start', method);
	...
	/**
	 * @typedef {Object} SignedEndorseProposal
	 ...
	 *
	 * @param {SignedEndorseProposal} request signed endorse transaction proposal, this signed
	 * proposal would be send to peer directly.
	 * @param {number} timeout the timeout setting passed on sendSignedEndorseProposal
	 */
	async sendSignedEndorseProposal(request, timeout) {
		return Channel.sendSignedEndorseProposal(request, timeout);
	}
	 ...
	 * @param {number} timeout the timeout setting passed on sendSignedEndorseProposal
	 */
	static async sendSignedEndorseProposal(request, timeout) {
	 ...
	 */
	async generateCommitProposal(request) {
		logger.debug('generateCommitProposal - start :: channel %s', this._name);

		if (!request) {
			throw Error('Missing input request object on the generateCommitProposal() call');
		}
		// Verify that data is being passed in
		if (!request.proposalResponses) {
			throw Error('Missing "proposalResponses" parameter in transaction request');
	 ...
	 * @property {Buffer} signedProposal - Required. The signed commit request
	 * @property {Orderer|string} orderer - Optional. The orderer instance or string name
	 *                     of the orderer to operate. See {@link Client.getTargetOrderer}
	 */
	 ...
	 * @param {number} timeout the timeout setting passed on sendSignedEndorseProposal
	 */
	async sendSignedCommitProposal(request, timeout) {
