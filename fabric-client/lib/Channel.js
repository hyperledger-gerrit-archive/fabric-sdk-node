...
var Channel = class {
	...
	getGenesisBlock(request) {
		...
		try {
			...
			// if nothing passed in, try seeing if there are added orderers to this instance
			//  user may have added one or one may have been added by the network configuration
			if(!request.orderer) request.orderer = this.getOrderers()[0];
			// might be a string name of the orderer so use utils to get the orderer
			var orderer = clientUtils.getOrderer(request.orderer, request.name, this);
			if(!orderer) {
				throw new Error('Missing orderer assigned to this channel for the getGenesisBlock request');
			}
			// verify that we have transaction id
			else if(!request.txId) {
				throw new Error('Missing txId input parameter with the required transaction identifier');
			}
		} catch(err) {
	...
	joinChannel(request) {
		...
		// verify that we have transaction id
			...
			try {
				targets = this._getTargets(request.targets);
