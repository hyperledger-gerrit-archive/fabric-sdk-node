...
const Channel = class {
	...
	getGenesisBlock(request) {
		...
		const envelope = client_utils.toEnvelop(client_utils.signProposal(signer,seekPayload));
	...
	async _discover(request) {
		...
		const signed_request = client_utils.toEnvelop(client_utils.signProposal(signer,discovery_request));
	...
	async getChannelConfigFromOrderer() {
		...
		// building manually or will get protobuf errors on send
		let envelope = client_utils.toEnvelop(client_utils.signProposal(signer,seekPayload));
		...
		// building manually or will get protobuf errors on send
		envelope = client_utils.toEnvelop(client_utils.signProposal(signer,seekPayload));
		// this will return us a block
	...
	static buildEnvelope(clientContext, chaincodeProposal, endorsements, proposalResponse, use_admin_signer){
		...
		return client_utils.toEnvelop(client_utils.signProposal(signer,payload));
	}
	/**
