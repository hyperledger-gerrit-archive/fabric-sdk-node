...
const Channel = class {
	...
	async _lifecycleAction(command, request) {
		...
		const chaincode = request.chaincode;
	 ...
	 * @property {integer} request_timeout - The timeout value to use for this request
	 * @property {TransactionID} txId - Optional. Transaction ID to use for the query.
	 *           Required when using the admin idendity.
	 */

	/**
	 * Sends a QueryChaincodeDefintion request to one or more peers.
	 *
	...
	async queryChaincodeDefinition(request) {
		...
		if (!responses || !Array.isArray(responses)) {
			throw new Error('Payload results are missing from the chaincode definition query');
		}
		...
		responses.forEach((response) => {
			if (response instanceof Error) {
				...
				logger.error('%s - unknown or missing results in query :: %j', method, results);
				results.push(new Error(response));
			}
		});
