...
const Channel = class {
	 ...
	 *           and optional for [generateTransactionProposal]{@link Channel#generateTransactionProposal}
	 * @property {map} transientMap - Optional. <string, byte[]> map that can be
	...
	/**
	 * Generates the transaction proposal bytes for a transaction
	 *
	 ...
	 * the signed proposal to peer by [sendSignedTransactionProposal]
	 *
	 * so the user's private
	 * key would not be required at SDK side.
	 *
	 * @param {TransactionProposal} request chaincode invoke request
	 * @param {string} certificate HEX string for the PEM encoded certificate
	 */
	generateTransactionProposal(request, mspId, certificate, admin) {
		const method = 'generateTransactionProposal';
		logger.debug('%s - start', method);
	 ...
	 *
	 * @param {Buffer[]} signedProposal signed transaction proposal, this signed
	 * proposal would be send to peer directly.
	 */
	async sendSignedTransactionProposal(request, timeout) {
		return Channel.sendSignedTransactionProposal(request, timeout);
	}
	 ...
	 * proposal would be send to peer directly.
	 */
	static async sendSignedTransactionProposal(request, timeout) {
