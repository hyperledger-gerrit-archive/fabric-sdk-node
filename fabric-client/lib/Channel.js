...
const Channel = class {
	...
	constructor(name, clientContext) {
		...
		this._discovery_time = null;
		this._discovery_peer = null;
		this._use_discovery = sdk_utils.getConfigSetting('initialize-with-discovery', true);

		// setup the default endorsement handler
		const handler_path = sdk_utils.getConfigSetting('endorsement-handler-path', 'fabric-client/lib/impl/DefaultEndorsementHandler.js');
	...
	initialize(request) {
		...
		this._discovery_results = null;
		this._discovery_time = null;
		const use_discovery = this._use_discovery;

		if (request) {
			if(typeof request[0] === 'byte') {
				this.loadConfigUpdate(config_update);

				return Promise.resolve(true);
			} else {
				if(request.discover) {
					// user may be requesting discovery
		...
		if(use_discovery) {
			...
			return self._discover(discover_request).then((discover_results) => {
				...
				if(discover_results) {
					...
					// add orderers names
					if(discover_results.config && discover_results.config.orderers){
						for(let msp_id in discover_results.config.orderers) {
							logger.debug('%s - orderers msp:%s', method, msp_id);
							const endpoints = discover_results.config.orderers[msp_id].endpoints;
					...
					discover_results.timestamp = new Date();
					self._discovery_results = discover_results;
					self._discovery_peer = target_peer;
					self._discovery_time = discover_results.timestamp;
	 ...
	 */
	 getDiscoveryResults() {
		 if(this._discovery_results) {
			 ...
			 const now = new Date();
			 if(now - this._discovery_time) {
				 return this.refresh();
			 } else {
				 return Promise.resolve(this._discovery_results);
			 }
		 } else {
			 return Promise.resolve(); // not working with discovery or we have not been initialized
		 }
	 }
