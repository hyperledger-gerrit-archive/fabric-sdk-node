...
//						logger.debug('queryTransaction - transaction ID :: %s:', channel_header.tx_id);
	 ...
	 * have the certificates to establish trust for this channel.
	 * The verifications performed are
	 *   - validate that the proposal signer is trusted
	 *   - verify that the proposal signer actually signed
	 *
	 * @param {ProposalResponse} The endorsement response from the peer,
	 *         includes the endorser and signature
	 ...
	 */
	 verifyProposalResponse(proposal_response) {
		...
		if (!msp){
			throw new Error(util.format('Failed to locate an MSP instance matching the requested id "%s" ', mspid));
		}
		logger.debug('verifyProposalResponse - found endorser\'s MSP');

		try {
			var identity = msp.buildIdentity(sid);
			if(!identity) {
