...
var Chain = class {
	...
	getOrganizationUnits() {
		...
		if(msps) {
			...
			for(var key in keys) {
				...
				var msp_orgs = msp.getOrganizationUnits();
				orgs = orgs.concat(msp_orgs);
			}
		}
	...
	loadConfigGroup(group, name, found_MSPs, top, keep_children) {
		logger.debug('loadConfigGroup - %s - START groups',name);
		if(!group) {
		...
		if(groups) {
			...
			for(let i =0; i < keys.length; i++) {
				...
				if(key === 'Application' || keep_children) {
					this.loadConfigGroup(groups.map[key], name+'.'+key, found_MSPs, false, true);
				}
			}
		}
		else {
		...
		if(values) {
			...
			for(let i =0; i < keys.length; i++) {
				...
				this.loadConfigValue(config_value, name, found_MSPs);
			}
		}
		else {
		...
		if(policies) {
			...
			for(let i =0; i < keys.length; i++) {
				...
				this.loadConfigPolicy(config_policy, name);
			}
		}
		else {
	 ...
	 */
	loadConfigValue(config_value, group_name, found_MSPs) {
		...
		try {
			...
			logger.debug('loadConfigValue - %s - name: %s - *** unable to parse value %s', group_name, config_value.key, err);
		}
	 ...
	 * @see /protos/common/configtx.proto
	 */
	loadConfigPolicy(config_policy, group_name) {
