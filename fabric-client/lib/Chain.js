...
var Chain = class {
	 ...
	 */
	setPrimaryPeer(peer) {
		this._primary_peer = peer;
	}

	/**
	 * Get the primary peer
	 * The peer to use for doing queries. When the set
	 * method has not been used to explicitly set the primary
	 * peer then the first peer on the list will be used by
	 * default.
...
//					chainCreatorsItem,
	...
	queryBlock(blockNumber) {
		...
		logger.info('************* got passed the id check');
		var request = {
			targets: [this.getPrimaryPeer()],
			chaincodeId : 'qscc',
			chainId: '',
