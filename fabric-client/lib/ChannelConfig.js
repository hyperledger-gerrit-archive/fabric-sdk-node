...
var ChannelConfig = class {
	...
	buildReadSetGroup(versions) {
		...
		proto_read_set_group.setVersion(this.getVersion(version_key, false, 0));
	...
	buildVersionOnlyValue(value, version_key) {
		...
		proto_read_set_value.setVersion(this.getVersion(version_key, false, 0));
	...
	buildVersionOnlyPolicy(policy, version_key) {
		...
		proto_read_set_policy.setVersion(this.getVersion(version_key, false, 0));
	...
	buildWriteSetGroup(version_key) {
		...
		write_set_group.setVersion(this.getVersion(version_key, false, 1));
		...
		proto_config_value.setVersion(this.getVersion(version_key + '.values.Consortium', true, 1));
...
//			logger.debug('loadConfigValue - %s   - value: %s', group_name, config_value.value.value);
	...
	buildConfigPolicy(name, policy, version_key) {
		...
		var not_known = true;
		if(name === 'Admins' || name === 'Writers' || name === 'BlockValidation') {
			not_known = false;
		}
		var increase_by = 1;
		// so if we do not not this policy name and this is a create
		if(not_known) increase_by = 0;
