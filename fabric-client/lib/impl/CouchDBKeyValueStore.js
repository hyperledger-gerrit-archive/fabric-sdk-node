	...
	getValue(name) {
		...
		return new Promise(function(resolve, reject) {
			self._database.get(name, function(err, body) {
				...
				if (err) {
					if (err.error !== 'not_found') {
						logger.error(util.format('getValue: ' + name + ', ERROR: [%s.get] - ',self._name), err.error);
					...
					logger.debug(util.format('getValue: ' + name + ', Retrieved message from %s.',self._name));
	...
	setValue(name, value) {
		...
		return new Promise(function(resolve, reject) {
			...
			self._database.get(name, function(err, body) {
				...
				if (err) {
					if (err.error !== 'not_found') {
						logger.error(util.format('setValue: ' + name + ', ERROR: [%s.get] - ',self._name), err.error);
						reject(err.error);
					} else {
					...
					logger.debug(util.format('setValue: ' + name + ', Retrieved entry from %s.',self._name));
	...
	_dbInsert(options) {
		...
		return new Promise(function(resolve,reject) {
			self._database.insert(options, function(err, body, header) {
				if (err) {
					logger.error(util.format('setValue, _dbInsert, ERROR: [%s.insert] - ',self._name), err.error);
					reject(new Error(err.error));
				} else {
					logger.debug(util.format('setValue, _dbInsert, Inserted member into %s.',self._name));
					resolve(true);
				}
			});
		});
