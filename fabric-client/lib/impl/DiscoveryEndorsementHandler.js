...
class DiscoveryEndorsementHandler extends api.EndorsementHandler {
	 ...
	 *  - sort group list by block height (larger on top) or randomly
	 *  - walk sorted list
	 *      -- put the preferred peers & organizations in the priority bucket if block height acceptable
	...
	_modify_groups(required, preferred, ignored, required_orgs, preferred_orgs, ignored_orgs, preferred_height_gap, sort, endorsement_plan) {
		...
		for (const group_name in endorsement_plan.groups) {
			...
			// get the highest block height if needed
			let highest = null;
	...
	_removePeers(ignored_peers, ignored_orgs, required_peers, required_orgs, peers) {
		...
		for (const peer of peers) {
			let found = ignored_peers[peer.name];
			if (!found) {
				found = ignored_orgs[peer.mspid];
				if (!found) {
					if (required_peers.count || required_orgs.count) {
						// peer must be on one required list
						found = required_peers[peer.name];
						if (!found) {
							found = required_orgs[peer.mspid];
						}
						if (!found) {
	...
	_splitList(preferred_peers, preferred_orgs, preferred_height_gap, highest, sorted_list) {
		...
		for (const peer of sorted_list) {
			let found = preferred_peers[peer.name];
			if (!found) {
				found = preferred_orgs[peer.mspid];
			}
			if (found && preferred_height_gap) {
				...
				if (highest.subtract(peer.ledger_height).greaterThan(preferred_height_gap)) {
					found = null; // this peer should not be on the priority list
				}
			} else {
