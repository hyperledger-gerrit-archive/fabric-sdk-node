...
var CryptoSuite_ECDSA_AES = class extends api.CryptoSuite {
	...
	generateKey(opts) {
		...
		if (typeof opts !== 'undefined' && typeof opts.ephemeral !== 'undefined' && opts.ephemeral === true) {
			...
			if (!this._cryptoKeyStore) {
				reject(new Error('generateKey opts.ephemeral is false, which requires CryptoKeyStore to be set.'));
			}
	...
	importKey(raw, opts, storeKey) {
		...
		if (!!store_key && !this._cryptoKeyStore) {
			reject(new Error('importKey storeKey is true, which requires CryptoKeyStore to be set.'));
		}
	...
	getKey(ski) {
		...
		if (!self._cryptoKeyStore) {
			reject(new Error('getKey requires CryptoKeyStore to be set.'));
		}
		return new Promise((resolve, reject) => {
			self._cryptoKeyStore._getKeyStore()
			.then ((st) => {
