...
var CryptoSuite_ECDSA_AES = class extends api.CryptoSuite {
	...
	importKey(raw, opts, storeKey) {
		...
		try {
			...
			// may not be a standard format, such as '----BEGIN ECDSA PRIVATE KEY----'
			// need to manually parse it by first converting to DER
			let hexString = pemToDER(pemString);

			try {
				key = KEYUTIL.parsePrivateRawECKeyHexAtObj(hexString);
			} catch(err) {
