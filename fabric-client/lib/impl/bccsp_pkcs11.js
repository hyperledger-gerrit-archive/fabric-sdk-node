...
class CryptoSuite_PKCS11 extends api.CryptoSuite {
	...
	constructor(keySize, hash, opts) {
		...
		if (!pkcs11Slot && isNaN(pkcs11Slot)) pkcs11Slot = utils.getConfigSetting('crypto-pkcs11-slot');
		if (!pkcs11Slot && isNaN(pkcs11Slot)) throw new Error(__func() + 'PKCS11 slot must be specified');
