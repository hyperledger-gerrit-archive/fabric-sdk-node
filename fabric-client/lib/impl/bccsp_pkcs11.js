...
var CryptoSuite_PKCS11 = class extends api.CryptoSuite {
		...
		return new Promise(function (resolve, reject) {
			try {
				...
				if (typeof handle.secretKey !== 'undefined') {
					...
					key = new aesKey({ ski: ski, key: handle.secretKey },
						self._keySize);
				}
				/*
				 * ECDSA key.
				 */
					...
					key = new ecdsaKey({ ski: ski, ecpt: attr.ecpt, pub: handle.publicKey, priv: handle.privateKey },
						self._keySize);
				}
				/*
				 * Put key in the session cache and return
	...
	encrypt(key, plainText, opts) {
		if (opts) {
			logger.info('The opts argument is not supported');
		}
	...
	importKey(pem, opts) {
		...
		if (typeof pem === 'undefined' || pem === null || !(pem instanceof Buffer || typeof pem === 'string'))
			return Promise.reject(Error(__func() + 'raw must be Buffer type or String type'));
		if (opts === null || opts.algorithm === null || typeof opts.algorithm !== 'string')
		...
		case 'AES':
			return new Promise(function (resolve, reject) {
				try {
					if (pem.length != (256 / 8)) throw new Error(
						__func() + 'AES key size must be 256 (bits)');

					var attr = self._pkcs11CreateObject(
						self._pkcs11, self._pkcs11Session, pem,
						token);
						/*
						 * Put key in the session cache and return
						 * promise of the key.
