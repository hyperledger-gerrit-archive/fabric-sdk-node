...
var EventHub = class {
	...
	_startHeartbeatTimer() {
		...
		if(heartbeat_time <= 0) {
			logger.debug('_startHeartbeatTimer -  not starting timer set to:%s',heartbeat_time);
			return;
		}
		...
		this._heartbeat_timer = setTimeout(() => {
			...
			if(self._activity_received) {
				...
				logger.debug('heartbeat timer - send back failure to all callbacks');
	...
	_connect(force) {
		...
		this._stream.on('data', function(event) {
			...
			if (event.Event == 'block') {
				...
				logger.debug('on.data - keep alive event received');
				self._keep_alive_response = true;
			}
			else {
	 ...
	 */
	_sendKeepAlive() {
		logger.debug('_sendKeepAlive - start');
		var user = this._clientContext.getUserContext();
		var signedEvent = new _events.SignedEvent();
		var event = new _events.Event();
