...
var EventHub = class {
	 ...
	 * @param {boolean} force - will setup a new call even
	 *                  a call is connected
	 */
	connect(force) {
	...
	registerChaincodeEvent(ccid, eventname, callback, error) {
		...
		var have_error_cb = error ? true : false;
	...
	registerBlockEvent(callback, error) {
		...
		// event hub if is not connected
		if(have_error_cb) {
			this.blockRegistrantsError.put(this.block_registrant_count, error);
			this._checkConnection(false, this.force_reconnect);
		}
	}
	...
	registerTxEvent(txid, callback, error) {
		...
		// event hub if is not connected
		if(have_error_cb) {
			this.txRegistrantsError.put(txid, error);
			this._checkConnection(false, this.force_reconnect);
		}
	}
	...
	_sendTxCallbacks(block) {
		...
		for (var index=0; index < block.data.data.length; index++) {
			...
			logger.debug('_sendTxCallbacks - txid=%s  val_code=%s', channel_header.tx_id, val_code);
