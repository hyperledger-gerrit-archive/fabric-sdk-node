...
var EventHub = class {
	...
	_startHeartbeatTimer() {
		...
		this._heartbeat_timer = setTimeout(() => {
			...
			if(self._activity_received) {
				...
				logger.error('heartbeat timer - send back failure to all callbacks');
	...
	_sendRegistration(register) {
		...
		logger.info('_sendRegistration - ***** here is the mspid %s',user.getIdentity().getMSPId());
