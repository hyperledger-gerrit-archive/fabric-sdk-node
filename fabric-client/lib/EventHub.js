...
var EventHub = class {
	...
	_connect(force) {
		...
		var send_timeout = setTimeout(function(){
			logger.error('connect - timed out after:%s', eh.ep._request_timeout);
			eh.disconnect();
		}, eh.ep._request_timeout);
		...
		this.call.on('data', function(event) {
			...
			var state = eh.call.call.channel_.getConnectivityState();
			logger.debug('connect - on.data - grpc stream state :%s',state);
			if (event.Event == 'block') {
				var block = Block.decodeBlock(event.block);
				eh._processBlockOnEvents(block);
				eh._processTxOnEvents(block);
				eh._processChainCodeOnEvents(block);
			}
	 ...
	 * all listeners with an "onError" callback.
	 */
	disconnect() {
		this.connected = false;
	...
	_checkConnection(throw_error, force_reconnect) {
		var state = 0;
		if(this.call) {
			state = this.call.call.channel_.getConnectivityState();
		}
		if(this.connected) {
			logger.debug('_checkConnection - this hub %s is connected with call channel state %s', this.ep.getUrl(), state);
		}
		else {
			logger.debug('_checkConnection - this hub %s is not connected with call channel state %s', this.ep.getUrl(), state);
			if(throw_error) {
	...
	registerBlockEvent(onEvent, onError) {
		...
		this.blockOnEvents.put(this.block_registrant_count, onEvent);

		// when there is an error callback try to reconnect this
		// event hub if is not connected
		if(have_error_cb) {
			this.blockOnErrors.put(this.block_registrant_count, onError);
			this._checkConnection(false, this.force_reconnect);
		}
	}
	 ...
	 * @param {function} onEvent Function to unregister
	 */
	unregisterBlockEvent(onEvent) {
		logger.debug('unregisterBlockEvent - start');
		var user = this._clientContext.getUserContext();
		if(!onEvent) {
			throw new Error('Missing "onEvent" parameter');
		}
		var keys = this.blockOnEvents.keys();
		logger.debug('unregisterBlockEvent - keys %s',keys);
		for(var i in keys) {
			var key = keys[i];
			logger.debug('unregisterBlockEvent - removing key %s',key);
			if(this.blockOnEvents.get(key) === onEvent ) {
				this.blockOnEvents.remove(key);
				this.blockOnErrors.remove(key);
			}
		}
	}
	...
	_processBlockOnEvents(block) {
		...
		if(this.blockOnEvents.size() == 0) {
			logger.debug('_processBlockOnEvents - no registered block events');
			return;
		}
	...
	_processTxOnEvents(block) {
		...
		if(this.transactionOnEvents.size() == 0) {
			logger.debug('_processTxOnEvents - no registered transaction events');
			return;
		}
	...
	_processChainCodeOnEvents(block) {
		...
		if(this.chaincodeRegistrants.size() == 0) {
			logger.debug('_processChainCodeOnEvents - no registered chaincode events');
			return;
		}
