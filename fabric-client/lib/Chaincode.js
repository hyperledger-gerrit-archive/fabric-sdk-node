...
const Chaincode = class {
	...
	async install(request) {
		...
		try {
			...
			const responses = await client_utils.sendPeersProposal(request.targets, proposal.signed, request.request_timeout);

			let i = 0;
			// loop on each peer in the target list
			for (const response of responses) {
				const peer = request.targets[i++];
				logger.debug('%s - looking at response from peer %s', method, peer);
