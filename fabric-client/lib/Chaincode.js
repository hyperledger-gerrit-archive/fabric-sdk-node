 ...
 *
 *
 * // define the chaincode for channel
 ...
 *   //  This step will not succeed until enough organizations have voted
 *   //  (step 6) for this chaincode definition to run on this
 ...
 *   const results = await mychannel.sendTransaction(orderer_request);
 *
 * // step 8: initialize the chaincode
 *   // This step will start the chaincode container and run the 'init' method
 *   // of the chaincode with the provided arguments.
 *   // This step will only be required when the code package is new or
 *   // changes and a new chaincode container must be initialized.
 *   const tx_id = client.newTransactionID();
 *   const request = {
...
const Chaincode = class {
	...
	setSequence(sequence) {
		if (!sequence) {
			// TODO try to query to find current value and increment
			sequence = 1;
		}
