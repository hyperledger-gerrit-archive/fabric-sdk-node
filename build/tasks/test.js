...
gulp.task('run-test', ['lint', 'run-test-mocha', 'run-full'],
	() => {
		return gulp.src('./test/features/*').pipe(cucumber({
			'steps': './test/features/steps/*.js',
			'support': './test/features/support/*.js',
...
gulp.task('run-test-headless', ['lint', 'run-headless', 'mocha-fabric-client', 'mocha-fabric-network'],
	() => {
		return gulp.src(['./fabric-ca-client/test/**/*.js'], { read: false })
			.pipe(mocha({ reporter: 'list', exit: true }));
	}
...
// - integration (e2e)
// - unit
gulp.task('run-full', ['clean-up', 'pre-test', 'compile', 'docker-ready', 'ca'],
	() => {
...
gulp.task('run-headless', ['clean-up', 'pre-test', 'ca'],
	() => {
