...
function shouldRunTests(tests) {
	// first check to see if they want to test PKCS11
	if (typeof process.env.PKCS11_TESTS === 'string' && process.env.PKCS11_TESTS.toLowerCase() === 'true') {
	...
	// check to see if they do not want to test PKCS11
		...
		tests.push('!test/unit/pkcs11.js');
		tests.push('!test/integration/network-e2e/e2e-hsm.js');
	// default to running the PKCS11 test so need to disable the non HSM version
	} else {
		tests.push('!test/integration/network-e2e/e2e.js');
	}
	// keep the java tests
