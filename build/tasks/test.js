...
gulp.task('run-test', ['clean-up', 'lint', 'run-test-mocha', 'run-tape-all'],
	() => {
		return gulp.src('./test/features/*').pipe(cucumber({
			'steps': './test/features/steps/*.js',
			'support': './test/features/support/*.js',
...
gulp.task('run-test-headless', ['clean-up', 'lint', 'run-tape-unit', 'mocha-fabric-client', 'mocha-fabric-network'],
	() => {
		return gulp.src(['./fabric-ca-client/test/**/*.js'], { read: false })
			.pipe(mocha({ reporter: 'list', exit: true }));
	}
...
// - integration (e2e)
// - unit
gulp.task('run-tape-all', ['pre-test', 'compile', 'docker-ready', 'ca'],
	() => {
...
gulp.task('run-tape-unit', ['pre-test', 'ca'],
	() => {
